...
begin s:='abcdef';P(s[2],s[2]);WriteLn(s,',',Length(s))end.`,
    output: ['aCcd,4'],
  },
  {
...
begin WriteLn(Choice(-1),',',Choice(6))end.`,
    output: ['7,12'],
  },
];
...
export const rejectedCases: ReferenceCase[] = [
  ...
  [
    'distinct-enum-array-index',
    'type A=(a1,a2);B=(b1,b2);var x:array[A]of Integer;begin x[b1]:=4 end.',
  ],
  [
    'distinct-enum-set-membership',
    'type A=(a1,a2);B=(b1,b2);S=set of A;var x:S;begin x:=[a1];WriteLn(b1 in x)end.',
  ],
  [
    'var-string-capacity-mismatch',
    "type S=String[3];var v:S;procedure P(var s:String);begin s:='abcdef'end;begin P(v)end.",
  ],
  [
    'constant-used-as-var-argument',
    'const n=7;procedure P(var x:Integer);begin x:=8 end;begin P(n)end.',
  ],
  [
...
export function generatedCases(count = 32): ReferenceCase[] {
  ...
  return Array.from({ length: count }, (_, caseIndex) => {
    ...
    const dividend = random(2001) - 1000,
      divisor = random(13) + 1;
...
WriteLn(u,',',v,',',w);i:=${String(dividend)};j:=${String(divisor)};WriteLn(i div j,',',i mod j)end.`,
      output: [
        sorted.join(','),
        String(values.reduce((sum, value) => sum + value * value, 0)),
        `${String(union.length)},${String(intersection.length)},${String(difference.length)}`,
