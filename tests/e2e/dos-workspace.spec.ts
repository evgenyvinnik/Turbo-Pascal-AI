...
test('DOS runs real COM instructions, DEBUG assembles and traces actual x86 registers, and files persist', async ({
  ...
  await expect(workspace.getByRole('button', { name: 'Examples', exact: true })).toBeEnabled({
    timeout: 30_000,
  });
  ...
  await expect(page.getByLabel('DOS output')).toContainText('Hello from real x86 DOS code!', {
    timeout: 20_000,
  });
  ...
  await ide.openMenu('f');
  await ide.chooseItem('d');
  await expect(workspace.getByRole('button', { name: 'Save files', exact: true })).toBeEnabled({
    timeout: 30_000,
  });
  await canvas.click();
  await page.keyboard.type('type x86test.txt', { delay: 60 });
  ...
  await page.keyboard.type('echo saved by exit> exit.txt', { delay: 60 });
  await page.keyboard.press('Enter');
...
test('native DOS Pascal compiles and executes inline x86 assembly and refuses to run stale binaries after an error', async ({
  page,
}) => {
  test.setTimeout(180_000);
  const ide = await Ide.open(page);
  await ide.typeSource(
    "program Native; var value: Word; begin asm mov ax,$1234; add ax,2; mov value,ax; end; writeln('NATIVE ASM: ',value); writeln('INTEGER BYTES: ',SizeOf(Integer)); end."
  );
  await ide.openMenu('f');
  await ide.chooseItem('d');
  const workspace = page.getByRole('region', { name: 'DOS workspace' });
  await expect(workspace.getByRole('button', { name: 'Run Pascal', exact: true })).toBeEnabled({
    timeout: 30_000,
  });
  await workspace.getByRole('button', { name: 'Run Pascal', exact: true }).click();
  await expect(page.getByLabel('DOS output')).toContainText('NATIVE ASM: 4662', {
    timeout: 100_000,
  });
  ...
  await expect(page.getByLabel('DOS output')).toContainText(
    'Compilation failed. The program was not run.',
    { timeout: 100_000 }
  );
