...
export async function compareRaster(
  ...
  return page.evaluate(
    async ({ actualUrl, referenceUrl, masks, scale }) => {
      ...
      const decode = async (url: string, width: number, height: number) => {
        ...
        if (image.naturalWidth !== width || image.naturalHeight !== height) {
          throw new Error(
            `Expected ${String(width)}×${String(height)} image, got ${String(image.naturalWidth)}×${String(image.naturalHeight)}`
          );
        }
      ...
      const [actual, reference] = await Promise.all([
        decode(actualUrl, width, height),
        decode(referenceUrl, 720, 400),
      ]);
      ...
      for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
          ...
          const different =
            ...
            [0, 1, 2, 3].some(
              (channel) =>
                actual.data[offset + channel] !== reference.data[referenceOffset + channel]
            );
      ...
      return {
        ...
        compared,
        cells: counts
          .flatMap((pixels, i) => (pixels ? [{ x: i % 80, y: Math.floor(i / 80), pixels }] : []))
          .sort((a, b) => b.pixels - a.pixels),
        map: Array.from({ length: 25 }, (_, y) =>
          counts
            .slice(y * 80, (y + 1) * 80)
            .map((n) => (n ? '#' : '.'))
            .join('')
...
export function rasterReport(name: string, diff: RasterDiff, masks: RasterMask[]): string {
  return [
    ...
    ...masks.map(
      (mask) =>
        `Mask x${String(mask.x)} y${String(mask.y)} ${String(mask.w)}×${String(mask.h)}: ${mask.reason}`
    ),
    '',
    'Mismatched cells (#):',
