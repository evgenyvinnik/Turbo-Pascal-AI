...
test('imported units, include constants, initialization and virtual object dispatch execute together', async ({
  page,
}) => {
...
test('recompilation uses an unsaved unit buffer instead of its imported disk copy', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
...
test('a semantic error in a used unit focuses that unit rather than the main program', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
...
test('run-time errors preserve a used unit filename and line in the dialog and source editor', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
...
test('run-time errors inside included statements map back to the include rather than its expanded line', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
...
test('a breakpoint is qualified by unit file and watches resolve that unit private scope', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
...
begin Secret:=7 end.`,
  ...
  await ide.moveTo(9, 1);
  await ide.press('Control+F8');
  await runCompiled(ide);
  await expect(ide.row(1)).toContainText('TWO.PAS');
  await expect(ide.row(23)).toContainText('9:1');
  await ide.press('Control+F7');
  await ide.type('Secret');
  await ide.press('Enter');
  await ide.waitForText('Secret: 7');
  await ide.press('Control+F7');
  await ide.type('SecretConstant');
  await ide.press('Enter');
  await ide.waitForText('SecretConstant: 9');
  await ide.press('Alt+F3');
  await ide.press('Control+F9');
  await ide.waitForText('SECOND=16');
  await ide.waitForText('FIRST UNIT');
});

test('a standalone unit compiles, but Run does not execute its initialization as a main program', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await importProject(
  ...
  await ide.waitForDialog('Compiling');
  await ide.press('Enter');
  await ide.waitForDialog('Error');
  await ide.waitForText('Cannot run a unit.');
  await ide.waitForText('Open a program that uses this unit.');
});

test('Boolean and range Compiler Options persist and source switches override execution without changing saved settings', async ({
  page,
}) => {
  const ide = await Ide.open(page);
...
begin
  ...
  await ide.press('Enter');
  await ide.waitForText('CALLS=1');
  await ide.press('Alt+F3');
  await ide.moveTo(1, 1);
  await ide.type('{$B-}{$R-}');
  await runCompiled(ide);
  await ide.waitForText('CALLS=0');
  await ide.waitForText('BYTE=0');
  await ide.openMenu('O');
  await ide.chooseItem('c');
  await ide.waitForText('[X] Complete boolean eval');
  await ide.waitForText('[X] Range checking');
...
test('strict VAR string Compiler Option persists and a source override rejects a mismatched argument', async ({
  page,
}) => {
  const ide = await Ide.open(page);
...
begin S:='x';Change(S);WriteLn('STRING=',S)end.`);
  ...
  await runCompiled(ide);
  await ide.waitForText('STRING=OK');
  await ide.press('Alt+F3');
  await ide.moveTo(1, 1);
  await ide.type('{$V+}');
  await ide.press('F9');
