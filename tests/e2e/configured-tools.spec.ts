...
async function addTool(ide: Ide, title: string, program: string, params: string): Promise<void> {
  ...
  await ide.waitForDialog('New Tool');
  await ide.type(title);
  await ide.press('Tab');
  await ide.type(program);
  await ide.press('Tab');
  await ide.type(params);
  await ide.press('Enter');
  await ide.waitForDialog('Tools');
  await ide.press('Alt+k');
}

test('a configured custom tool appears in the real menu and survives a browser reload', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await addTool(ide, 'List sources', 'DIR', '*.PAS > LIST.TXT');
  await ide.openMenu('T');
  await ide.waitForText('List sources');
  ...
  await expect(dos.getByRole('button', { name: 'Save files', exact: true })).toBeEnabled({
    timeout: 30_000,
  });
...
test('deleting a configured default removes its menu item and reassigns the first shortcut', async ({
  page,
}) => {
  const ide = await Ide.open(page);
  await ide.openMenu('O');
