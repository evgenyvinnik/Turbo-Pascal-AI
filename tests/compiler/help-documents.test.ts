...
import { getHelpDocument, helpLinkAt, identifierTopic, REFERENCE_TOPICS } from '../../src/components/IDE/helpDocuments';
...
describe('Help documents and cross references', () => {
  ...
  test('retains the supplied visible editor pages and adds useful continuation', () => {
    expect(getHelpDocument('edit').lines.slice(0, 32)).toEqual([...REFERENCE_HELP['Help-Edit-Window']!, ...REFERENCE_HELP['Help-2']!]);
    expect(getHelpDocument('edit').lines.slice(32).join('\n')).toContain('Shift+Tab selects the previous link');
  });
  ...
  test.each([['CONSTRUCTOR', 'constructor'], ['OBJECT', 'object'], ['USES', 'uses'], ['INTERFACE', 'unit'], ['$IFDEF', '$define'], ['ORD', 'numeric-routines'], ['WriteLn', 'writeln']])('looks up %s', (identifier, expected) => {
  ...
  test('the index contains all authored topics, including late-added language sections', () => {
    ...
    expect(index).toEqual(expect.arrayContaining(['unit', 'object', 'asm', 'statements', 'labels', 'comments', 'compiler-errors', 'runtime-errors']));
  });

  test('prose is not a link and context links stay on the actual subject', () => {
    expect(helpLinkAt('unit', 3, 0)).toBeUndefined();
    expect(getHelpDocument('context:Turbo-Help-Compiler-Options').links[0]?.target).toBe('directives');
    expect(getHelpDocument('context:Turbo-Help-Evaluate-and-Modify').links[0]?.target).toBe('evaluate');
  });
});
