...
const tools = ['Grep', 'Turbo Assembler', 'Turbo Debugger', 'Turbo Profiler'].map((title) => ({
  title,
  program: '',
  params: '',
}));
...
describe('configured Tools menu', () => {
  test('preserves default visual labels and hints while resolving saved indices', () => {
    ...
    expect(items.map(({ label, shortcut, hint }) => ({ label, shortcut, hint }))).toEqual(
      defaults.slice(4).map((item) => {
        const { label, shortcut, hint } = item as MenuEntry;
        return { label, shortcut, hint };
      })
    );
  ...
  test('renaming and appending a tool changes the actual menu and avoids accelerator collisions', () => {
    ...
    const items = buildToolsMenuItems(changed, defaults).filter(
      (item): item is MenuEntry => !('separator' in item)
    );
    expect(items.at(-1)?.id).toBe('tools.custom.4');
    expect(items.at(-1)?.label.replaceAll('~', '')).toBe('Disk listing');
    expect(
      items.map((item) => /~(.)~/.exec(item.label)?.[1]?.toLowerCase()).filter(Boolean).length
    ).toBe(
      new Set(items.map((item) => /~(.)~/.exec(item.label)?.[1]?.toLowerCase()).filter(Boolean))
        .size
    );
    expect(items.find((item) => item.id === 'tools.custom.0')?.label.replaceAll('~', '')).toBe(
      'Messages scan'
    );
  });
