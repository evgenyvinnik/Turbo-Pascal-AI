...
import {
  ...
  startDosRuntime,
  typeDosCommand,
} from '../../src/services/dos/dosRuntime';
...
describe('DOS keyboard', () => {
  it('maps browser keys to the GLFW codes js-dos expects', () => {
    ...
    expect(codes.map(dosKeyCode)).toEqual([
      65, 90, 48, 57, 290, 301, 320, 329, 335, 257, 256, 259, 265, 340,
    ]);
  });
  ...
  it('types commands through the keyboard, holding Shift only for shifted characters', async () => {
    ...
    const ci = {
      sendKeyEvent: (code: number, pressed: boolean) => {
        events.push(`${pressed ? '+' : '-'}${String(code)}`);
      },
    ...
    expect(events.join(' ')).toBe(
      [
        ...
        '+73 -73',
        '+82 -82',
        '+32 -32', // D (shifted), i, r, space
        '+340 +67 -67 -340',
        '+340 +59 -59 -340',
        '+257 -257', // C and : (shifted ;), Enter
      ].join(' ')
    );
  });
});
...
describe('native Pascal build batch', () => {
  const lines = (...args: Parameters<typeof nativePascalBatch>) =>
    nativePascalBatch(...args).split('\r\n');
  ...
  it('never runs a stale executable after a failed compile', () => {
    ...
    expect(at('if errorlevel 1 goto failed')).toBeGreaterThan(
      at('D:\\PPC386.EXE @D:\\NATIVE.CFG "D:\\SOURCE\\HELLO.PAS"')
    );
    ...
    expect(batch.slice(at(':failed'))).toEqual([
      ':failed',
