...
} from '../../src/compiler/errors/diagnostics';
...
describe('Borland diagnostic presentation', () => {
  ...
  test.each([
    ...
    ['Range check error (1..3)', 201],
    ['Array index 3 out of bounds (1..2)', 201],
    ['Stack overflow', 202],
    ['Heap overflow', 203],
    ['Invalid or disposed pointer', 204],
    ['Real overflow', 205],
    ['Arithmetic overflow', 215],
    ['File not found: DATA.TXT', 2],
    ['I/O error 103', 103],
    ...
    expect(formatPascalDiagnostic(diagnostic)).toMatch(
      new RegExp(`^Run-time error ${String(code)}: `)
    );
  });
  ...
  test('retains the originating unit or include source file', () => {
    ...
    expect(describePascalDiagnostic(error, 'compiler')).toMatchObject({
      code: 26,
      sourceFile: 'LIB/COUNTER.PAS',
      lineNumber: 12,
      columnNumber: 4,
    });
  });
