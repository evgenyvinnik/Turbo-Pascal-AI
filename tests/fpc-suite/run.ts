...
const reportPath = path.resolve(
  process.env.FPC_SUITE_REPORT ?? 'artifacts/verification/fpc-suite.json'
);
...
const DIALECT_DIRECTIVE =
  /\{\$(?:mode\s+(?!tp\b)\w+|modeswitch\b|h\+|longstrings\s+on|macro\s+on|coperators\s+on|inline\s+on|z[+\-\d]|minenumsize\b|j[+-]|writeableconst\b|asmmode\b|setpe(?:opt)?flags\b)/i;
...
const FPC_ONLY_TYPE =
  /\b(?:cardinal|smallint|shortint64|int64|qword|longword|dword|codepointer|ansistring|widestring|unicodestring|textfile|sizeint|sizeuint|ptrint|ptruint|nativeint|nativeuint)\b/i;
...
const FPC_ONLY_SYNTAX: readonly (readonly [RegExp, string])[] = [
  ...
  [/\/\//, 'uses // comments'],
  [
    /;\s*(?:cdecl|stdcall|safecall|cppdecl|mwpascal|softfloat|local)\s*;/i,
    'uses a Free Pascal procedure directive',
  ],
...
const SWITCH_OPTION: Record<string, keyof CompilerSwitches> = {
  '-Cr': 'rangeChecking',
  '-Co': 'overflowChecking',
  '-Ci': 'ioChecking',
};
...
const readSource = async (file: string) =>
  (await readFile(file, 'latin1')).replace(/^\xEF\xBB\xBF/, '');
...
function skipReason(header: ReadonlyMap<string, string>, fpcVersion: string): string | undefined {
  for (const [directive, reason] of Object.entries(UNSUPPORTED))
    if (header.has(directive)) return reason;
  const minimum = header.get('VERSION'),
    maximum = header.get('MAXVERSION');
  if (minimum && compareVersions(fpcVersion, minimum) < 0) return `needs Free Pascal ${minimum}`;
  if (maximum && compareVersions(fpcVersion, maximum) > 0)
    return `needs Free Pascal ${maximum} or earlier`;
  return undefined;
}

function translateOptions(
  value: string
): { fpc: string[]; switches: Partial<CompilerSwitches> } | string {
  const fpc: string[] = [],
  ...
  try {
    await execute(
      ...
      [
        '-Mtp',
        ...options,
        `-Fu${path.join(suite, 'tstunits')}`,
        `-Fu${path.dirname(file)}`,
        `-Fi${path.dirname(file)}`,
        `-FU${work}`,
        `-FE${work}`,
        `-o${executable}`,
        file,
    ...
    return {
      ...
      ...(reported.site ? { site: reported.site } : {}),
    };
  }
}
...
function runWithFpc(
  executable: string,
  work: string
): Promise<{ exitCode: number; output: string[] } | { failure: string }> {
  return new Promise((resolve) => {
    const child = execFile(
      executable,
      [],
      { cwd: work, timeout: 10_000, maxBuffer: 4 * 1024 * 1024 },
      (error, stdout) => {
        if (!error) resolve({ exitCode: 0, output: normalize(stdout) });
        else if (typeof error.code === 'number' && !error.killed)
          resolve({ exitCode: error.code, output: normalize(stdout) });
...
function runInBrowserEngine(
  ...
  try {
    ...
    const [file, text] =
      unit === undefined
        ? [filename, source]
        : [`${path.dirname(filename)}/FPCSUITE.PAS`, `program FpcSuite; uses ${unit}; begin end.`];
    bytecode = compileProject(text, file, {
      sources,
      unitDirectories: ['UNITS'],
      ...switches,
    }).bytecode;
  } catch (error) {
    ...
    if (error instanceof PascalError) {
      const file =
        'sourceFile' in error && typeof error.sourceFile === 'string' ? error.sourceFile : filename;
      return {
        kind: 'rejected',
        message: error.message,
        site: { file: path.basename(file), line: error.lineNumber },
      };
    }
  ...
  if (machine.getState() === MachineState.WAITING)
    return { kind: 'error', message: 'waits for input' };
...
async function judge(
  ...
  const outcome: Outcome = {
    test: relative,
    directives: Object.fromEntries(header),
    verdict: { category: 'skipped', reason: '' },
  };
  ...
  if (typeof options === 'string')
    return { ...outcome, verdict: { category: 'skipped', reason: options } };
  ...
  if (dialect)
    return {
      ...
      verdict: {
        category: 'excluded',
        reason: `not Turbo Pascal: the source sets ${dialect[0].toLowerCase()}}`,
      },
    };
  ...
  if (fpcType)
    return {
      ...
      verdict: {
        category: 'excluded',
        reason: `not Turbo Pascal 7: uses the Free Pascal type ${fpcType[0].toLowerCase()}`,
      },
    };
  ...
  if (syntax)
    return {
      ...outcome,
      verdict: { category: 'excluded', reason: `not Turbo Pascal 7: ${syntax[1]}` },
    };
  ...
  try {
    ...
    if (expected.failsToCompile && build.built)
      return {
        ...outcome,
        verdict: { category: 'excluded', reason: 'Free Pascal accepts it in TP mode' },
      };
    if (!expected.failsToCompile && !build.built)
      return {
        ...outcome,
        verdict: {
          category: 'excluded',
          reason: 'not Turbo Pascal: Free Pascal rejects it in TP mode',
        },
      };
    ...
    if (!expected.failsToCompile && !expected.compileOnly) {
      ...
      if ('failure' in run)
        return {
          ...outcome,
          verdict: { category: 'excluded', reason: `Free Pascal's program ${run.failure}` },
        };
      ...
      if (run.exitCode !== expected.exitCode)
        return {
          ...
          verdict: {
            category: 'excluded',
            reason: `Free Pascal's program exits ${String(run.exitCode)}, the test expects ${String(expected.exitCode)}`,
          },
        };
      reference = run;
    }

    const vm = runInBrowserEngine(
      source,
      relative.toUpperCase(),
      sources,
      options.switches,
      expected,
      isUnit ? path.basename(file).replace(/\.\w+$/, '') : undefined
    );
    if (vm.kind === 'asm')
      return {
        ...outcome,
        verdict: {
          category: 'excluded',
          reason: 'uses inline assembly, which only the native DOS compiler runs',
        },
      };
    if (vm.kind === 'crashed')
      return {
        ...outcome,
        vm: { built: false, message: vm.message },
        verdict: { category: 'fail', reason: 'the browser compiler crashed' },
      };
    if (vm.kind === 'rejected') {
      outcome.vm = { built: false, message: vm.message, line: vm.site.line };
      if (!expected.failsToCompile)
        return {
          ...outcome,
          verdict: { category: 'fail', reason: 'the browser compiler rejects it' },
        };
      if (!build.site)
        return {
          ...outcome,
          verdict: {
            category: 'pass',
            reason: 'rejected, as the test requires (Free Pascal gives no location)',
          },
        };
      return samePlace(vm.site, build.site)
        ? {
            ...outcome,
            verdict: { category: 'pass', reason: 'rejected where Free Pascal rejects it' },
          }
        : {
            ...outcome,
            verdict: { category: 'fail', reason: 'rejected, but not where Free Pascal rejects it' },
          };
    }
    if (expected.failsToCompile)
      return {
        ...
        verdict: {
          category: 'fail',
          reason: 'the browser compiler accepts a program that must not compile',
        },
      };
    if (vm.kind === 'built')
      return { ...outcome, vm: { built: true }, verdict: { category: 'pass', reason: 'compiles' } };
    if (vm.kind === 'error')
      return {
        ...outcome,
        vm: { built: true, message: vm.message },
        verdict: { category: 'fail', reason: `the program does not finish: ${vm.message}` },
      };
    ...
    return vm.exitCode === expected.exitCode
      ...
      : {
          ...
          verdict: {
            category: 'fail',
            reason: `exits ${String(vm.exitCode)}, Free Pascal exits ${String(expected.exitCode)}`,
          },
        };
  } finally {
...
for (const directory of directories) {
  ...
  const tests = (await readdir(folder))
    .filter((name) => /^t.*\.(pp|pas)$/i.test(name) && (!filter || name.includes(filter)))
    .sort();
  let next = 0;
  await Promise.all(
...
const count = (category: Verdict['category']) =>
  outcomes.filter((outcome) => outcome.verdict.category === category).length;
const judged = count('pass') + count('fail');
const tally = (category: Verdict['category'], key: (outcome: Outcome) => string) => {
  const totals = new Map<string, number>();
  for (const outcome of outcomes.filter((o) => o.verdict.category === category))
    totals.set(key(outcome), (totals.get(key(outcome)) ?? 0) + 1);
...
const KINDS: [Kind, string][] = [
  ['run', 'Must run and exit as expected'],
  ['compile', 'Must compile'],
  ['reject', 'Must be rejected where Free Pascal rejects it'],
];
const byKind = Object.fromEntries(
  KINDS.map(([kind]) => {
    const judgedOfKind = outcomes.filter(
      (o) => o.kind === kind && (o.verdict.category === 'pass' || o.verdict.category === 'fail')
    );
    return [
      kind,
      {
        judged: judgedOfKind.length,
        passed: judgedOfKind.filter((o) => o.verdict.category === 'pass').length,
      },
    ];
  })
) as Record<Kind, { judged: number; passed: number }>;
const summary = {
  suite: `${pin.repository} ${pin.tag} (${pin.commit.slice(0, 12)})`,
  directories,
  freePascal: fpcVersion,
  programs: outcomes.length,
  skipped: count('skipped'),
  excluded: count('excluded'),
  judged,
  passed: count('pass'),
  failed: count('fail'),
  harnessErrors: count('error'),
  byKind,
};
await mkdir(path.dirname(reportPath), { recursive: true });
await writeFile(
  reportPath,
  JSON.stringify({ ...summary, generatedAt: new Date().toISOString(), outcomes }, null, 2) + '\n'
);

console.log(`Free Pascal ${fpcVersion} tests, ${pin.tag}: ${directories.join(', ')}`);
console.log(
  `${String(outcomes.length)} programs: ${String(summary.skipped)} skipped, ${String(summary.excluded)} excluded by Free Pascal, ${String(judged)} judged`
);
console.log(`${String(summary.passed)}/${String(judged)} pass in the browser compiler and VM:`);
for (const [kind, title] of KINDS)
  console.log(
    `  ${String(byKind[kind].passed).padStart(4)}/${String(byKind[kind].judged).padEnd(4)} ${title}`
  );
console.log('');
for (const [title, category] of [
  ...
  ['Excluded', 'excluded'],
  ['Failed', 'fail'],
  ['Harness errors', 'error'],
] as const) {
  const reasons = tally(category, (o) => o.verdict.reason);
  if (!reasons.length) continue;
  console.log(`${title}:`);
  for (const [reason, n] of reasons.slice(0, 8))
    console.log(`  ${String(n).padStart(4)}  ${reason}`);
}
console.log('\nMost common browser-compiler rejections:');
for (const [message, n] of tally('fail', (o) =>
  o.verdict.reason === 'the browser compiler rejects it' ? shape(o.vm?.message) : ''
)
  .filter(([m]) => m)
  .slice(0, 12))
...
const regressions = baseline.passing.filter((test) =>
  ['fail', 'error'].includes(verdicts.get(test)?.category ?? '')
);
const unjudged = baseline.passing.filter(
  (test) => !['pass', 'fail', 'error'].includes(verdicts.get(test)?.category ?? '')
);
const newPasses = outcomes
  .filter((outcome) => outcome.verdict.category === 'pass' && !listed.has(outcome.test))
  .map((outcome) => outcome.test);
console.log(
  `\nBaseline: ${String(baseline.passing.length - unjudged.length)} of ${String(baseline.passing.length)} listed passes judged here`
);
if (newPasses.length)
  console.log(`  ${String(newPasses.length)} newly passing: ${newPasses.join(', ')}`);
if (process.env.FPC_SUITE_UPDATE) {
  const passing = [
    ...new Set([
      ...unjudged,
      ...outcomes.filter((o) => o.verdict.category === 'pass').map((o) => o.test),
    ]),
  ].sort();
  await writeFile(baselinePath, JSON.stringify({ ...baseline, passing }, null, 2) + '\n');
  console.log(
    `  Wrote ${String(passing.length)} passing tests to ${path.relative(process.cwd(), baselinePath)}`
  );
