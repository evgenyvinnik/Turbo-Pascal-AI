...
unit's `FileRec` and `TextRec`, original overlay/linker formats, `.BGI`
loading, or all compiler switches. An `absolute` variable of another type shares
its variable's cells rather than its bytes, and a store through a pointer to a
field of a variant case does not reach the other cases. Timer interrupts tick only
