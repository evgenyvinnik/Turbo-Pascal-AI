...
const COMPILE_PATTERNS: readonly (readonly [RegExp, number])[] = [
  ...
  [/^(?:Undeclared (?:identifier|variable|procedure or function)|Unknown type)\b/, 3],
  [/^Duplicate (?:identifier|field)\b/, 4],
  [/^Unexpected character:|^Unexpected token after program:/, 5],
  [/^Invalid number: missing exponent digits$/, 6],
  [/^Invalid hexadecimal integer$|^Integer constant out of range$/, 7],
  [/^Unterminated string$/, 8],
  [/^Unterminated comment$|^Unexpected end of file$/, 10],
  [/^(?:Unknown unit|Unit source not found|Include file not found|File not found)\b/, 15],
  [/^Invalid compiler directive\b/, 17],
  [/^Unknown pointer target type$/, 19],
  [/^Variable required$/, 20],
  [/^Expected type,/, 21],
  [/^Set range must be within 0\.\.255$/, 23],
  [/^String capacity must be/, 25],
  [
    /^Type mismatch|^VAR string parameters require matching capacities$|^Incompatible comparison operands$/,
    26,
  ],
  [/^Invalid subrange bounds$/, 27],
  [/^(?:Subrange|Case range) lower bound exceeds upper bound$/, 28],
  [/^Set elements must have an ordinal type$/, 29],
  [/^(?:Constant expected|Constant expression expected)/, 31],
  [/^Numeric constant expression expected$/, 32],
  [/^Typed pointer required$/, 33],
  [/^Functions returning arrays or records are not supported$/, 34],
  [/^Label expected$|^Undeclared label$/, 35],
  [/^Integer operands required$/, 38],
  [/^Ordinal set element required$|^Case selector must be ordinal$/, 39],
  [/^Boolean expression expected$/, 40],
  [
    /^IN requires|^Invalid set operator$|^AND, OR and XOR require|^NOT requires|^Numeric operands required/,
    41,
  ],
  [/^Unexpected token in expression:|^Invalid constant expression$/, 42],
  [/^Unknown record field\b/, 44],
  [/^Data segment too large$/, 49],
  [/^Too many variables$/, 96],
  [/^Expected 'to' or 'downto',/, 58],
  [/^Unresolved forward declaration\b/, 59],
  [/^Ordinal typecast required$|^Typecast requires one value$|^Invalid typecast/, 61],
  [/^Division by zero/, 62],
  ...
  [/^Pointer variable required$/, 65],
  [/^Concat requires strings$/, 67],
  [/^Circular (?:unit|interface) reference\b/, 68],
  [/^Unit name mismatch\b/, 69],
  [/^WITH requires a record$/, 75],
  [/^Set element out of range$|^Character code out of range$/, 76],
  [/^File variable required$/, 77],
  [/^Numeric operand expected$|^Str requires a numeric argument$/, 79],
  [/^Duplicate label definition$/, 81],
  [/^Undefined label$/, 82],
  [/^Inc and Dec require an ordinal variable$/, 104],
  [/^Arithmetic overflow$/, 108],
  [/^\w+ is only valid inside a loop$/, 109],
...
const RUNTIME_PATTERNS: readonly (readonly [RegExp, number])[] = [
  [/^File not found\b/, 2],
  [/^Invalid file name$|^Path not found\b/, 3],
  [/^Read past end of file$/, 100],
  [/^Disk full$/, 101],
  [/^File is not assigned$/, 102],
  [/^File is not open$/, 103],
  [/^File is not open for reading$/, 104],
  [/^File is not open for writing$/, 105],
  [
    /^Invalid (?:integer|real|boolean) input:|^Invalid (?:number|boolean) in file$|^(?:Integer|Real) input out of range$/,
    106,
  ],
  [/^Division by zero$/, 200],
  [
    /^Range check error|^Array index .* out of bounds|^String index out of bounds$|^Set element out of range$|^Character code out of range$/,
    201,
  ],
  [/^Stack overflow$/, 202],
  [/^Heap overflow$/, 203],
...
export function describePascalDiagnostic(
  error: unknown,
  phase: PascalDiagnostic['phase']
): PascalDiagnostic {
  ...
  const sourceFile =
    error instanceof PascalError && 'sourceFile' in error && typeof error.sourceFile === 'string'
      ? error.sourceFile
      : undefined;
  return {
    phase,
