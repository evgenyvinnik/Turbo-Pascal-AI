...
export function encodeBinary(
  ...
  layout.forEach((cell, index) => {
    ...
    if (cell.kind === 'string') {
      ...
      for (let i = 0; i < characters.length; i++) bytes[offset + i + 1] = characters.charCodeAt(i) & 255;
      ...
      const number = Math.trunc(
        cell.kind === 'char' ? String(value).charCodeAt(0) || 0
          : cell.kind === 'pointer' && memory.pointerBits ? memory.pointerBits(value)
            : Number(value)
      );
      if (cell.bytes <= 4) {
...
export function decodeBinary(
  ...
  layout.forEach((cell, index) => {
    ...
    if (cell.kind === 'string') {
      ...
      if (memory.setCharacters) characters = String.fromCharCode(...bytes.subarray(offset + 1, offset + cell.bytes));
    } else if (cell.kind === 'set') {
    ...
    if (characters !== undefined) memory.setCharacters?.(address + (cell.offset ?? index), characters);
    offset += cell.bytes;
  });
}
...
function cellStarts(layout: BinaryCell[]): Float64Array {
  ...
  if (!sums) {
    ...
    layout.forEach((cell, index) => { sums![index + 1] = sums![index]! + cell.bytes; });
...
export function cellAtByte(layout: BinaryCell[], byte: number): { index: number; at: number } | undefined {
  const sums = cellStarts(layout);
  if (byte < 0 || byte >= sums[layout.length]!) return undefined;
  let low = 0, high = layout.length - 1;
  while (low < high) {
...
function withOffsets(layout: BinaryCell[]): BinaryCell[] {
  ...
  if (!cells) {
    cells = layout.every((cell) => cell.offset !== undefined) ? layout : layout.map((cell, index) => ({ ...cell, offset: cell.offset ?? index }));
 ...
 * first starts at. */
function span(layout: BinaryCell[], start: number, length: number): { cells: BinaryCell[]; from: number } {
  const sums = cellStarts(layout);
  if (start < 0 || start + length > sums[layout.length]!) throw new PascalError('Access beyond the variable');
  if (length <= 0) return { cells: [], from: start };
  const first = cellAtByte(layout, start)!.index, last = cellAtByte(layout, start + length - 1)!.index;
