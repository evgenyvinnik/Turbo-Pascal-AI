...
interface CompiledUnit {
  ...
  state: 'interface' | 'implementation' | 'ready';
  debugStart?: number;
  debugEnd?: number;
}
...
export class Compiler {
  ...
  compile(root: Node, options: CompilerOptions = {}): Bytecode {
    if (root.type === NodeType.UNIT) {
      ...
      return this.compile(
        ...
        },
        {
          ...options,
          resolveUnit: (name) =>
            name.toLowerCase() === unit.name.toLowerCase() ? unit : options.resolveUnit?.(name),
        }
      );
    }
    ...
    this.bytecode.dataSegment = this.dataSegment([
      ...this.initializationOrder.flatMap((unit) => unit.scope.locals),
      ...this.scope.locals,
    ]);
  ...
  private importUnitDeclarations(unit: string): void {
    ...
    if (!exports) {
      const program = new Parser(
        new Lexer(new Stream(`program ${unit}; ${source} begin end.`))
      ).parse();
  ...
  private loadUnit(name: string, node: Node): CompiledUnit {
    const key = name.toLowerCase(),
      existing = this.units.get(key);
    if (existing) {
      if (existing.state === 'interface')
        this.fail(node, `Circular unit interface dependency "${name}"`);
      return existing;
    }
    const source = this.compilerOptions.resolveUnit?.(name);
    if (source === undefined) this.fail(node, `Unknown unit "${name}"`);
    const unitNode =
      typeof source === 'string' ? new Parser(new Lexer(new Stream(source))).parseUnit() : source;
    if (unitNode.name.toLowerCase() !== key)
      this.fail(unitNode, `Unit name "${unitNode.name}" does not match "${name}"`);
    const parent = this.scope,
      scope = this.newScope(null, unitNode.name);
    ...
    for (const [name, symbol] of scope.symbols)
      if (!builtinNames.has(name)) unit.exports.set(name, symbol);
  ...
  private qualified(node: Node): Node {
    if (node.type !== NodeType.FIELD_ACCESS || (node.record as Node).type !== NodeType.IDENTIFIER)
      return node;
    const name = String((node.record as Node).name),
  ...
  private lookup(name: string): Symbol | undefined {
    for (const record of [...this.scope.withRecords].reverse()) {
      ...
      if (field?.privateOwner && !this.inModule(field.privateOwner))
        this.fail({ type: NodeType.IDENTIFIER }, `Private object member "${name}"`);
      if (field)
        return {
          kind: 'variable',
          name,
          ...
          ...(record.constant === undefined
            ? {}
            : { readOnly: true, constantName: record.constant }),
        };
    }
  ...
  private declare(name: string, symbol: Symbol, node: Node): void {
    if (
      this.scope.methodOwner?.fields?.has(name.toLowerCase()) ||
      this.scope.methodOwner?.object?.methods.has(name.toLowerCase())
    )
    ...
    if (
      existing &&
      ![...this.standardSymbols.values()].some((unit) => [...unit.values()].includes(existing))
    )
  ...
  private addressVariable(variable: Variable): void {
    ...
    if (variable.view) {
      ...
      this.emitView(
        () => {
          this.addressVariable(target);
        },
        () => {
          this.emitLayoutOf(target);
        },
        variable.type,
        0,
        ...
        if (symbol?.kind === 'type')
          return (
            ...
            call.arguments.every(
              (argument) =>
                this.constantExpression(argument) &&
                this.expressionType(argument).kind !== 'pointer'
            )
          );
  ...
  private exactReal(type: PascalType): boolean {
    return (
      type.kind === 'real' &&
      (type.comp === true || (type.byteSize === 10 && type.approximate !== true))
    );
  }
  ...
  private coprocessorResult(left: PascalType, right: PascalType): PascalType {
    return this.exactContext || this.exactReal(left) || this.exactReal(right)
      ? EXTENDED
      : INTERMEDIATE;
  }
  ...
  private compatible(target: PascalType, source: PascalType, reference = false): boolean {
    if (target.procedureSignature || source.procedureSignature) {
      if (target.procedureSignature && source.procedureSignature)
        return this.proceduralCompatible(target, source);
      return !reference &&
        ((target.procedureSignature && source.kind === 'pointer' && !source.base) ||
          (source.procedureSignature && target.kind === 'pointer' && !target.base))
        ? true
        : false;
    }
    if (target.kind === source.kind) {
      ...
      if (target.kind === 'pointer')
        return (
          ...
          (!reference && this.descendsFrom(source.base, target.base))
        );
      ...
      if (reference && target.kind === 'string')
        return (
          Boolean(target.openString) === Boolean(source.openString) &&
  ...
  private requireType(node: Node, target: PascalType, source: PascalType, reference = false): void {
    ...
    if (
      ...
      target.kind === 'string' &&
      source.kind === 'string' &&
      (target.openString || node.strictVarStrings === false)
    )
      return;
      ...
      switch (node.type) {
        ...
        case NodeType.VAR_DECLARATION: {
          ...
          const names = new Set(
            declaration.names
              ...
              .filter((name) => {
                ...
                return existing !== undefined && standard.includes(existing);
              })
          );
          const mentions = (type: unknown): boolean =>
            typeof type === 'object' &&
            type !== null &&
            (((type as Node).type === NodeType.IDENTIFIER &&
              names.has(String((type as Node).name).toLowerCase())) ||
              Object.values(type).some((value) =>
                Array.isArray(value) ? value.some(mentions) : mentions(value)
              ));
          ...
          if (declaration.address) {
            ...
            const [segment, offset] = declaration.address.map(
              (part) => this.ordinalValue(this.constant(part).value) & 0xffff
            );
            // 0:0 is the interrupt table's first byte, not nil.
            const absolute =
              farAddress(segment ?? 0, offset ?? 0, this.bytecode.farSegments) || LINEAR_BASE;
          ...
          if (declaration.absolute) {
            ...
            const same =
              !found.view &&
              target.type.kind !== 'untyped' &&
              JSON.stringify(this.binaryLayout(target.type)) ===
                JSON.stringify(this.binaryLayout(type));
            for (const name of declaration.names)
              this.declare(
                name,
                same
        ...
        case NodeType.FUNCTION: {
          ...
          const type =
            declaration.type === NodeType.FUNCTION
              ? declaration.returnType
                ? this.resolveType(declaration.returnType)
                : previous?.kind === 'routine'
                  ? previous.type
                  : this.fail(node, 'Function return type required')
              : declaration.routineKind === 'constructor'
                ? BOOLEAN
                : VOID;
          ...
          if (declaration.interrupt) {
            ...
            if (declaration.type === NodeType.FUNCTION)
              this.fail(node, 'An interrupt routine must be a procedure');
            if (this.scope.parent || declaration.name.includes('.'))
              this.fail(node, 'Interrupt procedures cannot be nested or methods');
            if (
              parameters.length > 12 ||
              parameters.some(
                (parameter) =>
                  parameter.reference ||
                  parameter.type.kind !== 'integer' ||
                  parameter.type.byteSize !== 2
              )
            )
          ...
          if (
            ...
            if (previous.declaration.routineKind !== declaration.routineKind)
              this.fail(node, 'Routine kind does not match declaration');
  ...
  private parameterList(nodes: Node[]): Parameter[] {
    ...
    for (const parameterNode of nodes) {
      ...
      const reference = parameterNode.type === NodeType.VAR_PARAMETER || type.kind === 'untyped';
  ...
  private parameterType(parameter: Node): PascalType {
    ...
    const explicitOpenString =
      node.type === NodeType.IDENTIFIER &&
      String(node.name).toLowerCase() === 'openstring' &&
      !this.lookup(String(node.name));
    ...
    if (
      parameter.type === NodeType.VAR_PARAMETER &&
      type.kind === 'string' &&
  ...
  private proceduralType(routine: Routine): PascalType {
    return {
      ...POINTER,
      procedureSignature: { parameters: routine.parameters, result: routine.type },
    };
  }
  ...
  private descendsFrom(type: PascalType, ancestor: PascalType): boolean {
    for (let current: PascalType | undefined = type; current; current = current.object?.ancestor)
      if (
        current === ancestor ||
        (current.object !== undefined && current.object === ancestor.object)
      )
        return true;
    return false;
  }
  private resolveObject(node: Node, type: PascalType, name: string): void {
    if (this.scope.level !== 0)
      this.fail(node, 'Object types must be declared at program or unit level');
    ...
    const fields: NonNullable<PascalType['fields']> = new Map(
      ancestor?.fields ?? [['$vmt', { offset: 0, type: INTEGER }]]
    );
    Object.assign(type, {
      kind: 'record',
      size: ancestor?.size ?? 1,
      byteSize: ancestor?.byteSize ?? 0,
      fields,
      object: {
        id: this.objectTypes.length + 1,
        ancestor,
        hasVirtual: ancestor?.object?.hasVirtual ?? false,
        methods: new Map(ancestor?.object?.methods ?? []),
      },
    });
    ...
    for (const field of node.fields as VarDeclarationNode[]) {
      ...
      for (const fieldName of field.names) {
        ...
        fields.set(fieldName.toLowerCase(), {
          offset: type.size,
          type: fieldType,
          ...(field.privateMember ? { privateOwner: this.scope } : {}),
        });
        type.size += fieldType.size;
        type.byteSize += fieldType.byteSize;
      }
    }
    ...
    for (const method of node.methods as (ProcedureNode | FunctionNode)[]) {
      ...
      if (routine.virtual && method.routineKind === 'constructor')
        this.fail(method, 'Constructors cannot be virtual');
      ...
      if (inherited?.virtual) {
        if (
          !routine.virtual ||
          routine.dynamicIndex !== inherited.dynamicIndex ||
  ...
  private resolveType(node: Node): PascalType {
    switch (node.type) {
      case NodeType.PROCEDURAL_TYPE: {
        ...
        if (this.aggregate(result) || result.procedureSignature)
          this.fail(node, 'Invalid procedural function result type');
      ...
      case NodeType.STRING_TYPE: {
        ...
        return {
          ...
          capacity,
          byteSize: capacity + 1,
          openStringDeclaration: Boolean(node.openStrings) && capacity === 255,
        };
      }
      ...
      case NodeType.SET_TYPE: {
        ...
        return {
          ...
          low,
          high,
          byteSize: Math.floor(high / 8) - Math.floor(low / 8) + 1,
        };
      }
      ...
      case NodeType.SUBRANGE_TYPE: {
        ...
        const base =
          a.type.kind === 'integer' && !a.type.enumeration
            ? this.integerRangeType(lower, upper, node)
            : a.type;
      ...
      case NodeType.ARRAY_TYPE: {
        ...
        for (const indexNode of [...array.indexTypes].reverse()) {
          ...
          if (size < 1 || (high - low + 1) * element.byteSize > 65520)
            this.fail(node, 'Array size exceeds supported storage');
          element = {
            kind: 'array',
            size,
      ...
      case NodeType.RECORD_TYPE: {
        ...
        const place = (
          ...
          for (const [index, variantCase] of variant.cases.entries()) {
            for (const label of variantCase.labels) {
              const values =
                label.type === NodeType.RANGE ? [label.low as Node, label.high as Node] : [label];
              for (const value of values)
                this.requireType(value, tagType, this.constant(value).type);
            }
            const placed = place(variantCase.fields, variantCase.variant, cell, byteStart + bytes, [
              { part, case: index },
              ...variants,
            ]);
            cell = placed.cell;
            ...
            all.push(...placed.all);
            cases.push({
              cells: placed.storage
                .flatMap((entry) => this.cells(entry.type, entry.offset))
                .map((entry) => ({ offset: entry.offset, cell: this.binaryCell(entry.type) })),
        ...
        return {
          ...
          fields,
          ...(record.variant
            ? { layout: placed.storage, initialLayout: placed.all, variantParts: ownParts }
            : {}),
  ...
  private constant(node: Node): Constant {
    ...
    switch (node.type) {
        ...
        return {
          ...
          value: node.isReal
            ? this.coprocessorMode()
              ? coprocessorValue(
                  parseReal(String(node.text ?? node.value), node.lineNumber) ?? Number(node.value),
                  node.lineNumber
                )
      ...
      case NodeType.UNARY_OP: {
        ...
        if (
          unary.operator.toLowerCase() === 'not' &&
          value.type.kind === 'integer' &&
          this.numeric(value.type)
        ) {
          const number = ~Number(value.value);
      ...
      case NodeType.BINARY_OP: {
        ...
        if (['=', '<>', '<', '>', '<=', '>='].includes(operator)) {
          if (
            !(this.numeric(left.type) && this.numeric(right.type)) &&
        ...
        if (
          left.type.kind === 'boolean' &&
          right.type.kind === 'boolean' &&
          ['and', 'or', 'xor'].includes(operator)
        ) {
          const a = Boolean(left.value),
            b = Boolean(right.value);
          return {
            kind: 'constant',
            type: BOOLEAN,
        ...
        const value: Value | Float80 = real
          ? coprocessor
            ? extendedOperation(
                operator,
                this.realValue(left.value),
                this.realValue(right.value),
                node.lineNumber
              )
        ...
        return {
          ...
          type: real
            ? coprocessor
              ? this.coprocessorResult(left.type, right.type)
              : REAL
            : this.integerRangeType(Number(value), Number(value), node),
  ...
  private compileRoutine(routine: Routine): void {
    ...
    for (const parameter of routine.parameters) {
      ...
      if (untyped)
        type.untypedLayout = this.variable(`$layout_${parameter.name}`, INTEGER, declaration);
      ...
      if (type.openArray) {
        ...
        if (!parameter.reference && !parameter.readOnly)
          (this.scope.openCopies ??= []).push(variable);
      }
    }
    ...
    if (declaration.interrupt)
      this.bytecode.interruptHandlers[routine.proceduralId] = {
        ...
        parameters: routine.parameters.length,
      };
  ...
  private compileBody(block: BlockNode, routine: Routine | undefined): void {
    ...
    if (!routine && this.printerUsed) {
      const lst = {
        ...
        name: 'Lst',
        internalVariable: this.printerFile(),
      } as Node;
    ...
    if (routine && routine.type.kind !== 'void') {
      ...
      if (this.scope.constructorBody) {
        this.emit(Opcode.LDA, 0, 0);
        this.literal(true, BOOLEAN);
        this.emit(Opcode.STI, TypeCode.B);
      }
    }
    ...
    if (this.scope.nextOffset > FRAME_CELLS)
      this.fail(block, 'Frame storage exceeds supported size');
    this.bytecode.setOperand2(entry, this.scope.nextOffset);
    if (!routine)
      for (const unit of this.initializationOrder)
        this.recordDebugScope(
          unit.scope,
          unit.debugStart ?? entry,
          unit.debugEnd ?? entry,
          this.scope.nextOffset
        );
  ...
  private constantCall(call: CallNode): Constant {
    ...
    if (target?.kind === 'type' && argument && !second && this.ordinal(target.type)) {
      ...
      const value = Number(
        (target.type.low ?? 0) < 0
          ? BigInt.asIntN(Number(bits), number)
          : BigInt.asUintN(Number(bits), number)
      );
      const type = target.type;
      if (type.kind === 'char')
        return { kind: 'constant', type, value: String.fromCharCode(value) };
      if (type.kind === 'boolean') return { kind: 'constant', type, value: value ? 1 : 0 };
      return {
        kind: 'constant',
        type: type.enumeration ? type : this.integerRangeType(value, value, call),
        value,
      };
    }
    ...
    if (name === 'sizeof' || name === 'high' || name === 'low') {
      ...
      if (bounds.kind === 'char')
        return { kind: 'constant', type: CHAR, value: String.fromCharCode(value) };
      return {
        kind: 'constant',
        type:
          bounds.kind === 'integer' && !bounds.enumeration
            ? this.integerRangeType(value, value, call)
            : bounds,
        value,
      };
    }
    ...
    switch (name) {
      case 'abs':
        return value.type.kind === 'real'
          ? { ...value, value: Math.abs(number()) }
      ...
      case 'succ': {
        ...
        if (
          (value.type.low !== undefined && next < value.type.low) ||
          (value.type.high !== undefined && next > value.type.high)
        )
          this.fail(call, 'Constant out of range');
        if (value.type.kind === 'char')
          return { kind: 'constant', type: CHAR, value: String.fromCharCode(next) };
        return value.type.kind === 'integer' && !value.type.enumeration
          ? integer(next)
          : { ...value, value: next };
      }
      case 'round':
      case 'trunc': {
        const real = number();
        return integer(
          name === 'trunc' ? Math.trunc(real) : Math.sign(real) * Math.round(Math.abs(real))
        );
      }
      case 'ptr': {
        // A segment and offset: an address that finds its variable when it
        // is used, and keeps the segment for Seg. Ptr(0, 0) is nil.
        const segment = ordinal() & 0xffff,
          offset = (second ? this.ordinalValue(this.constant(second).value) : 0) & 0xffff;
        return {
          kind: 'constant',
          type: POINTER,
          value: farAddress(segment, offset, this.bytecode.farSegments),
        };
      }
      default:
  ...
  private typedConstantValue(
      ...
      for (const { name, value } of (node as RecordConstantNode).fields) {
        ...
        const otherCase = (field: RecordField) =>
          field.variants?.some((variant) =>
            given?.variants?.some(
              (chosen) => chosen.part === variant.part && chosen.case !== variant.case
            )
          );
        ...
        for (const variant of given.variants ?? [])
          stores.push({ offset, type: INTEGER, value: 0, sync: variant });
        next = index + 1;
      }
      return;
    }
    if (type.procedureSignature) {
      ...
      const routine =
        node.type === NodeType.IDENTIFIER ? this.lookup(String(node.name)) : undefined;
  ...
  private packedText(type: PascalType, node: Node): string | undefined {
    if (
      type.kind !== 'array' ||
      type.openArray ||
      type.element?.kind !== 'char' ||
      type.size < 2 ||
      !this.constantExpression(node)
    )
      return undefined;
    const value = this.constant(node);
    const text =
      this.text(value.type) || value.type.kind === 'char' ? String(value.value) : undefined;
  ...
  private packedString(type: PascalType): boolean {
    return (
      type.kind === 'array' && !type.openArray && type.element?.kind === 'char' && type.size >= 2
    );
  }
  ...
  private emitCharArrayPointer(node: Node): boolean {
    if (node.extendedSyntax === false) return false;
    const type = this.expressionType(node);
    if (type.kind !== 'array' || type.openArray || type.element?.kind !== 'char' || type.low !== 0)
      return false;
    this.address(node);
    return true;
  }
  ...
  private staticAddress(node: Node, type?: PascalType): number {
    ...
    if (
      type &&
      this.charPointer(type) &&
      node.extendedSyntax !== false &&
      node.type !== NodeType.ADDRESS_OF
    ) {
      const location = this.staticLocation(node);
      if (!this.zeroBasedChars(location.type, node))
        this.fail(node, 'Type mismatch: expected pointer');
  ...
  private staticStringCharacter(node: Node): (() => void) | undefined {
    ...
    if (position < 0 || position > (location.type.capacity ?? 255))
  ...
  private staticSegmentPart(node: Node): (() => void) | undefined {
    ...
    if ((part !== 'ofs' && part !== 'seg') || !operand || rest.length || this.lookup(name))
      return undefined;
  ...
  private initialize(type: PascalType, offset: number): void {
    if (type.kind === 'array') {
      ...
      this.literal(
        type.kind === 'char' ? '\0' : this.text(type) ? '' : type.kind === 'set' ? '[]' : 0,
        type
      );
  ...
  private emitStatement(node: Node): void {
    ...
    switch (node.type) {
      case NodeType.UNIT_INITIALIZATION: {
        const parent = this.scope,
          unit = node.unit as CompiledUnit;
        this.scope = unit.scope;
      ...
      case NodeType.WITH_STATEMENT: {
        ...
        // record up to date.
        for (const record of node.records as Node[])
          this.emitVariantSyncs(this.variantChain(record));
        return;
      }
  ...
  private assign(node: AssignmentNode): void {
    ...
    if (
      text !== undefined ||
      (this.charPointer(targetType) && this.emitCharArrayPointer(node.value))
    ) {
      if (text !== undefined) this.emitStaticText(text, node.value);
      this.emit(Opcode.STI, this.typeCode(targetType));
      return;
    }
    const sourceType = targetType.procedureSignature
      ? this.proceduralValue(node.value)
  ...
  private variantChain(node: Node): { base: Node; part: number; case: number }[] {
    ...
    if (node.type === NodeType.FIELD_ACCESS) {
      ...
      return [
        ...(field?.variants ?? []).map((variant) => ({ base: record, ...variant })),
        ...this.variantChain(record),
      ];
    }
    ...
    if (node.type === NodeType.IDENTIFIER) {
      ...
      const base = {
        ...
        pointer: {
          ...
          internalVariable: {
            ...symbol.container,
            type: { ...POINTER, base: symbol.containerType },
          },
        },
        exact: true,
      } as Node;
  ...
  private hasSideEffects(node: Node): boolean {
    ...
    if (node.type === NodeType.IDENTIFIER && !node.internalVariable)
      return this.lookup(String(node.name))?.kind === 'routine';
      ...
      Array.isArray(value)
        ? value.some(
            (item) =>
              ...
              item !== null &&
              'type' in item &&
              this.hasSideEffects(item as Node)
          )
        : typeof value === 'object' &&
          value !== null &&
          'type' in value &&
          this.hasSideEffects(value as Node)
    );
  }
  private replaceNode(node: Node, target: Node, replacement: Node): Node {
    ...
    for (const [key, value] of Object.entries(node))
      if (typeof value === 'object' && value !== null && 'type' in value)
        copy[key] = this.replaceNode(value as Node, target, replacement);
    return copy;
  }
  ...
  private withVariantSyncs<T>(
    targets: Node[],
    refresh: boolean[],
    compile: (targets: Node[]) => T
  ): T {
    const settled = targets.map((target) => {
      ...
      const replacement = {
        type: NodeType.POINTER_DEREF,
        pointer: { type: NodeType.IDENTIFIER, internalVariable: pointer },
  ...
  private emitVariantRefresh(
    target: { node: Node } | { variable: Variable },
    type: PascalType
  ): void {
  ...
  private writtenArguments(node: CallNode): number[] {
      ...
      indexes.filter((index) => {
        ...
        if (
          ...
          ![
            ...
            NodeType.POINTER_DEREF,
          ].includes(argument.type)
        )
          return false;
        try {
          return (
            this.variantChain(argument).length > 0 ||
            this.variantRefreshes(this.expressionType(argument), 0).length > 0
          );
        } catch {
          return false;
        }
    ...
    if (!builtin) {
      ...
      const parameters =
        symbol?.kind === 'variable' ? symbol.type.procedureSignature?.parameters : undefined;
    ...
    return candidates(
      known[name] ?? all.filter((index) => builtin.params[index]?.mode === ParamMode.VAR)
    );
  }
  ...
  private expressionType(node: Node, rawProcedure = false): PascalType {
    ...
    switch (node.type) {
      ...
      case NodeType.ADDRESS_OF: {
        ...
        return base.kind === 'untyped' || node.typedPointer !== true
          ? POINTER
          : { ...POINTER, base };
      }
      ...
      case NodeType.IDENTIFIER: {
        const symbol = this.lookup(node.name as string);
        if (symbol)
          return !rawProcedure && symbol.kind === 'variable' && symbol.type.procedureSignature
            ? symbol.type.procedureSignature.result
            : symbol.type;
        const method = this.methodTarget({
          ...node,
          type: NodeType.CALL,
      ...
      case NodeType.CALL: {
        ...
        if (call.receiver) {
          ...
          if (qualified.type === NodeType.IDENTIFIER)
            call = { ...call, receiver: undefined, name: String(qualified.name) };
        }
        ...
        if (call.name.toLowerCase() === 'new' && call.arguments[0])
          return this.expressionType(call.arguments[0]);
      ...
      case NodeType.FIELD_ACCESS: {
        ...
        if (method) {
          if (method.privateOwner && !this.inModule(method.privateOwner))
            this.fail(node, `Private object method "${access.field}"`);
          return method.type;
        }
        const field = type.fields?.get(access.field.toLowerCase());
        if (!field) this.fail(node, `Unknown record field "${access.field}"`);
        if (field.privateOwner && !this.inModule(field.privateOwner))
          this.fail(node, `Private object member "${access.field}"`);
        return !rawProcedure && field.type.procedureSignature
          ? field.type.procedureSignature.result
          : field.type;
      }
      case NodeType.UNARY_OP: {
      ...
      case NodeType.BINARY_OP: {
        ...
        if (
          binary.extendedSyntax !== false &&
          (operator === '+' || operator === '-') &&
          this.charPointer(left)
        )
          return left;
        if (
          binary.extendedSyntax !== false &&
          operator === '+' &&
          left.kind === 'integer' &&
          this.charPointer(right)
        )
          return right;
        if (operator === '/' || left.kind === 'real' || right.kind === 'real')
          return this.coprocessorArithmetic(left, right)
            ? this.coprocessorResult(left, right)
            : REAL;
  ...
  private emitExpression(node: Node): PascalType {
    ...
    if (node.internalVariable) {
      const variable = node.internalVariable as Variable;
      this.loadVariable(variable);
    ...
    switch (node.type) {
      ...
      case NodeType.ADDRESS_OF: {
        ...
        if (nilOffset !== undefined) {
          ...
          return node.typedPointer === true
            ? { ...POINTER, base: this.expressionType(node.operand as Node) }
            : POINTER;
        }
      ...
      case NodeType.POINTER_DEREF: {
        if (this.untypedPointer(this.expressionType(node.pointer as Node)))
          this.fail(node, 'Typed pointer required');
      ...
      case NodeType.IDENTIFIER: {
        ...
        if (symbol?.kind === 'variable') {
          if (symbol.type.procedureSignature)
            return this.proceduralCall(
              { ...node, type: NodeType.CALL, name: String(node.name), arguments: [] },
              true,
      ...
      case NodeType.ARRAY_ACCESS: {
        ...
        if (callbackType.procedureSignature)
          return this.proceduralCall(
            { ...node, type: NodeType.CALL, name: '$indirect', arguments: [] },
            true,
            { node, type: callbackType }
          );
        const access = node as ArrayAccessNode;
        if (this.expressionType(access.array).kind === 'string') {
          ...
          const variable =
            ...
            this.lookup(base.name as string)?.kind === 'variable';
          if (
            variable ||
            [NodeType.ARRAY_ACCESS, NodeType.FIELD_ACCESS, NodeType.POINTER_DEREF].includes(
              base.type
            )
          ) {
            this.address(node);
      ...
      case NodeType.UNARY_OP: {
        ...
        if (operator === 'not') {
          ...
          else if (type.kind === 'integer' && this.numeric(type))
            this.integerHelper('not', type, node, 1);
  ...
  private pcharOperands(
    ...
    if (node.extendedSyntax === false) return false;
    const leftArray = this.zeroBasedChars(left, node.left),
      rightArray = this.zeroBasedChars(right, node.right);
    if (!leftArray && !rightArray) return false;
    const leftPChar = leftArray || this.charPointer(left),
      rightPChar = rightArray || this.charPointer(right);
    ...
    return (
      ['=', '<>', '<', '>', '<=', '>='].includes(operator) &&
      (this.charPointer(left) || this.charPointer(right)) &&
      leftPChar &&
      rightPChar
    );
  }
  ...
  private binaryOperation(node: BinaryOpNode): PascalType {
    ...
    const leftType = this.expressionType(node.left),
      rightType = this.expressionType(node.right);
    // Two packed strings compare as strings only when they are as long.
    if (
      ['=', '<>', '<', '>', '<=', '>='].includes(operation) &&
      this.packedString(leftType) &&
      this.packedString(rightType) &&
      leftType.size !== rightType.size
    )
      this.fail(node, 'Type mismatch');
    if (
      !node.completeBooleanEvaluation &&
      ...
      if (operation === 'and') this.expression(node.right);
      else this.literal(true, BOOLEAN);
      const end = this.emit(Opcode.UJP);
      this.patch(branch);
      if (operation === 'and') this.literal(false, BOOLEAN);
    ...
    if (comparisons[operator] !== undefined) {
      ...
      if (left.kind === 'pointer' && right.kind === 'pointer')
        this.emit(Opcode.CSP, 2, InternalProcedure.NORMALIZE_POINTERS);
  ...
  private emitAddress(node: Node): PascalType {
    ...
    if (node.type === NodeType.POINTER_DEREF) {
      ...
      if (reinterpreted) {
        this.emitView(
          () => {
            this.address(reinterpreted.operand);
          },
          () => {
            this.literal(this.layoutId(reinterpreted.source), INTEGER);
          },
          reinterpreted.type,
          0,
          this.refreshListId(reinterpreted.source),
          this.variantChain(reinterpreted.operand)
        );
      ...
      if (
        !['untyped', 'file'].includes(pointer.base.kind) &&
        !pointer.base.object &&
        node.exact !== true
      ) {
    ...
    if (node.type === NodeType.ARRAY_ACCESS) {
      ...
      for (const index of access.indices) {
        ...
        if (type.openArray) {
          ...
          this.helper(`open-array-index-${String(line)}-${String(checked)}`, 2, (value, high) => {
            ...
            if (
              ...
              (checked && (indexValue < 0 || indexValue > Number(high)))
            )
        ...
        this.helper(
          `array-index-${String(low)}-${String(high)}-${String(line)}-${String(checked)}`,
          1,
          (value) => {
            const indexValue = typeof value === 'string' ? value.charCodeAt(0) : Number(value);
            if (
              !Number.isInteger(indexValue) ||
              (checked && (indexValue < low || indexValue > high))
            )
              throw new PascalError(
  ...
  private checkRange(type: PascalType, node: Node, source?: PascalType): void {
    if (type.kind === 'real') {
      const line = node.lineNumber ?? this.line;
      if (type.comp)
        this.helper(`comp-${String(line)}`, 1, (value) => compValue(value as Real, line));
      ...
      // result computed in doubles already is one.
        ...
        (!source ||
          (source.kind === 'real' &&
            source.byteSize === 10 &&
            (!source.approximate || source.extendedValue)) ||
          source.comp)
      )
    ...
    if (type.kind === 'string') {
      if (type.openCapacity) {
        this.stringCapacity(type);
        this.helper('open-string-copy', 2, (value, capacity) =>
          String(value).slice(0, Number(capacity))
        );
        return;
      }
    ...
    if (type.kind === 'set') {
      ...
      const checked = Boolean(node.rangeChecking);
      this.helper(
        `set-range-${String(low)}-${String(high)}-${String(line)}-${String(checked)}`,
        1,
        (value) => {
          if (checked && this.decodeSet(value).some((member) => member < low || member > high))
            throw new PascalError('Set element out of range', line);
          return JSON.stringify(
            this.decodeSet(value).filter((member) => member >= low && member <= high)
          );
        }
      );
      return;
    ...
    const low = type.low,
      ...
      line = node.lineNumber ?? this.line;
    const checked = Boolean(node.rangeChecking),
      bits = type.byteSize * 8,
      signed = low < 0;
    // Char and Byte share their range, but not what the check returns.
    this.helper(
      `range-${type.kind}-${String(low)}-${String(high)}-${String(line)}-${String(checked)}-${String(bits)}`,
      1,
      (value) => {
        ...
        const wrapped = Number(
          signed
            ? BigInt.asIntN(bits, BigInt(Math.trunc(number)))
            : BigInt.asUintN(bits, BigInt(Math.trunc(number)))
        );
  ...
  private call(node: CallNode, expression: boolean): PascalType {
    ...
    try {
      ...
      const builtin =
        !routine && !node.receiver && this.modules.lookupProcedure(node.name) !== undefined;
  ...
  private emitRoutineCall(
    ...
    if (
      !expression &&
      routine.type.kind !== 'void' &&
      routine.declaration.routineKind !== 'constructor'
    )
      return this.discardResult(node, routine.type, () =>
        this.emitRoutineCall(node, true, routine, bound)
      );
    ...
    routine.parameters.forEach((parameter, index) => {
      ...
      if (parameter.type.openArray) {
        ...
        this.requireType(
          { ...argument, strictVarStrings: node.strictVarStrings },
          parameter.type,
        ...
        if (text !== undefined) this.emitStaticText(text, argument);
          ...
          const source = parameter.type.procedureSignature
            ? this.proceduralValue(argument)
  ...
  private openArrayArgument(argument: Node, parameter: Parameter): void {
    ...
    if (type.kind !== 'array' || !type.element || !this.compatible(element, type.element, true))
      this.fail(
        argument,
        'Type mismatch: open array argument must be an array of its element type'
      );
  ...
  private exitChain(block: Node): void {
    ...
    const signature: PascalType = {
      ...POINTER,
      procedureSignature: { parameters: [], result: VOID },
    };
  ...
  private unsupportedAssembly(reason: string): never {
    throw new NativePascalRequired(
      this.sourceFile ?? this.programName,
      `${reason} requires the native DOS compiler`
    );
  }
  /** The function result the current routine returns, if any. */
  private resultVariable(): Variable | undefined {
    for (const symbol of this.scope.symbols.values())
      if (symbol.kind === 'variable' && symbol.result) return symbol;
    return undefined;
  }
  ...
  private nilOffset(node: Node): number | undefined {
    if (node.type === NodeType.FIELD_ACCESS) {
      ...
      const type = this.expressionType(access.record);
      const field =
        type.kind === 'record' ? type.fields?.get(access.field.toLowerCase()) : undefined;
    ...
    if (node.type === NodeType.ARRAY_ACCESS) {
      ...
      for (const index of access.indices) {
        if (
          ...
          type.kind !== 'array' ||
          !type.element ||
          !this.constantExpression(index)
        )
          return undefined;
        offset +=
          (this.ordinalValue(this.constant(index).value) - (type.low ?? 0)) * type.element.byteSize;
        type = type.element;
      }
      return offset;
    }
    ...
    const zero =
      value?.type === NodeType.NIL ||
      (value?.type === NodeType.NUMBER && Number(value.value) === 0);
    return cast?.kind === 'type' && cast.type.kind === 'pointer' && zero && !rest.length
      ? 0
      : undefined;
  }
  ...
  private byteOffset(type: PascalType, cellOffset: number): number {
    return this.cells(type)
      .filter((cell) => cell.offset < cellOffset)
      .reduce((sum, cell) => sum + cell.type.byteSize, 0);
  }
  ...
  private assemblyName(
    path: string[],
    line: number,
    pointers: Map<Variable, Variable>,
    assembler: boolean
  ): AsmName | undefined {
    ...
    if (symbol.kind === 'constant') {
      ...
      if (typeof value === 'string' && value.length === 1)
        return { kind: 'constant', value: value.charCodeAt(0) };
    ...
    if (symbol.container)
      return this.unsupportedAssembly('Fields reached through WITH or Self in assembler');
    if (symbol.type.openArray || symbol.type.openString)
      return this.unsupportedAssembly('Open parameters in assembler');
    const { type, offset } = fields(symbol.type, path.slice(1));
    ...
    const indirect =
      symbol.reference || (assembler && symbol.parameter === true && symbol.type.byteSize > 4);
    if (indirect) {
      let key = symbol;
      if (!symbol.reference) {
    ...
    if (symbol.type.kind === 'pointer' && symbol.type.base)
      variable.target = this.binaryLayout(symbol.type.base);
        ...
        type.kind === 'char'
          ...
          : type.kind === 'boolean'
            ...
            : type.kind === 'pointer'
              ...
              : 'integer';
      if (
        !['integer', 'char', 'boolean', 'pointer'].includes(type.kind) ||
  ...
  private inlineBlock(node: Node, variables: Variable[]): AsmBlock {
    ...
    for (const element of node.elements as { size?: 1 | 2; parts: Part[] }[]) {
      ...
      bytes.push({
        value: value & 0xff,
        ...(reference
          ? { variable: reference.index, displacement: reference.displacement + value }
          : {}),
      });
  ...
  private interruptCall(node: CallNode, builtin: BuiltinDef): PascalType {
    ...
    if (type !== this.unitType('dos', 'Registers'))
      this.fail(regs, 'Type mismatch: Registers expected');
    const line = node.lineNumber ?? this.line;
    ...
    const field = (index: number): Operand => ({
      kind: 'memory',
      size: 2,
      variable: 0,
      registers: [],
    ...
    const instructions: AsmBlock['instructions'] = [
      ...words.map((register, index) => ({
        mnemonic: 'mov',
        operands: [{ kind: 'register', register } as Operand, field(index)],
        line,
      })),
      ...
      ...words.map((register, index) => ({
        mnemonic: 'mov',
        operands: [field(index), { kind: 'register', register } as Operand],
        line,
      })),
    ];
  ...
  private declareStandard(standard: StandardVariable): void {
    ...
    const symbol = this.standardSymbol(standard.name, types[standard.kind]),
  ...
  private standardSymbol(name: string, type: PascalType): Variable {
    ...
    return {
      ...
      name,
      type,
      offset: this.standardVariable(name),
      scope: root,
      reference: false,
    };
  }
  ...
  private variableCast(
    node: Node
  ): { type: PascalType; operand: Node; source: PascalType } | undefined {
    ...
    const designator =
      ...
      (inner.type === NodeType.IDENTIFIER &&
        (inner.internalVariable !== undefined ||
          this.lookup(String(inner.name))?.kind === 'variable')) ||
      (inner.type === NodeType.CALL &&
        (this.variableCast(inner) !== undefined || this.untypedCast(inner) !== undefined));
    if (!designator) return undefined;
    const source = this.expressionType(operand);
    if (
      source.kind === 'untyped' ||
      source.kind === 'file' ||
      target.type.kind === 'file' ||
      target.type.object ||
      source.object
    )
      return undefined;
  ...
  private castView(cast: { type: PascalType; operand: Node; source: PascalType }): void {
    if (cast.type.byteSize !== cast.source.byteSize)
      this.fail(cast.operand, 'Invalid typecast: the variable and the type differ in size');
    if (
      JSON.stringify(this.binaryLayout(cast.type)) ===
      JSON.stringify(this.binaryLayout(cast.source))
    ) {
      this.address(cast.operand);
      return;
    }
    this.emitView(
      ...
      () => {
        this.literal(this.layoutId(cast.source), INTEGER);
      },
      cast.type,
      0,
      this.refreshListId(cast.source),
      this.variantChain(cast.operand)
    );
  }
  ...
  private designatorRoot(node: Node): Node | undefined {
    ...
    const shown = (type: PascalType) =>
      !['untyped', 'file', 'string'].includes(type.kind) &&
      !type.object &&
      !type.openArray &&
    ...
    if (node.type === NodeType.ARRAY_ACCESS) {
      ...
      return array.kind === 'array' && !array.openArray
        ? this.designatorRoot(node.array as Node)
        : undefined;
    }
    const type = this.expressionType(node);
    ...
    if (symbol.container) {
      ...
      return record && !record.object
        ? ({
            ...
            pointer: {
              type: NodeType.IDENTIFIER,
              internalVariable: { ...symbol.container, type: { ...POINTER, base: record } },
            },
            exact: true,
          } as Node)
        : undefined;
  ...
  private addressReinterpretation(
    node: Node
  ): { type: PascalType; operand: Node; source: PascalType } | undefined {
    if (node.type !== NodeType.CALL || (node as CallNode).receiver) return undefined;
    const {
    ...
    if (
      !argument ||
      rest.length ||
      argument.type !== NodeType.ADDRESS_OF ||
      target?.kind !== 'type'
    )
      return undefined;
    ...
    if (
      ...
      return undefined;
    if (JSON.stringify(this.binaryLayout(base)) === JSON.stringify(this.binaryLayout(source)))
      return undefined;
  ...
  private fileRecordCast(node: Node): { type: PascalType; operand: Node } | undefined {
    ...
    if (
      target.type !== this.unitType('dos', 'FileRec') &&
      target.type !== this.unitType('dos', 'TextRec')
    )
      return undefined;
    const pointer =
      argument.type === NodeType.POINTER_DEREF ? (argument.pointer as Node) : undefined;
    const operand = pointer?.type === NodeType.ADDRESS_OF ? (pointer.operand as Node) : argument;
    return this.expressionType(operand).kind === 'file'
      ? { type: target.type, operand }
      : undefined;
  }
  ...
  private untypedCast(node: Node): { type: PascalType; operand: Node } | undefined {
    ...
    const target = this.lookup(name);
    if (!operand || rest.length || target?.kind !== 'type') return undefined;
    return this.expressionType(operand).kind === 'untyped'
      ? { type: target.type, operand }
      : undefined;
  }
  ...
  private requireWritable(node: Node): void {
    ...
    if (constant !== undefined)
      this.fail(node, `Constant parameter "${constant}" cannot be modified`);
  }
  ...
  private constantRoot(node: Node): string | undefined {
    ...
    if (node.type === NodeType.ARRAY_ACCESS)
      return this.constantRoot((node as ArrayAccessNode).array);
    if (node.type === NodeType.FIELD_ACCESS)
  ...
  private proceduralValue(node: Node): PascalType {
    ...
    if (node.type === NodeType.IDENTIFIER) {
      ...
      if (symbol?.kind === 'routine') {
        if (symbol.parent.level !== 0 || symbol.methodOwner)
          this.fail(node, 'Procedural values require a non-nested procedure or function');
        if (!symbol.declaration.farCalls)
          this.fail(node, 'Procedural values require a FAR procedure or function');
        const type = this.proceduralType(symbol);
        this.literal(symbol.proceduralId, type);
        return type;
      }
    }
  ...
  private proceduralTarget(node: CallNode): { node: Node; type: PascalType } | undefined {
    ...
    if (node.receiver) {
      ...
      if (field?.type.procedureSignature)
        return {
          node: { ...node, type: NodeType.FIELD_ACCESS, record: node.receiver, field: node.name },
          type: field.type,
        };
      return undefined;
    }
  ...
  private proceduralCall(
    ...
    if (expression && signature.result.kind === 'void')
      this.fail(node, 'Procedure cannot be used as an expression');
    if (!expression && signature.result.kind !== 'void')
      return this.discardResult(node, signature.result, () =>
        this.proceduralCall(node, true, target)
      );
    if (node.arguments.length !== signature.parameters.length)
      this.fail(node, 'Wrong number of procedural arguments');
    const value = this.temp(POINTER);
    this.addressVariable(value);
    this.proceduralValue(target.node);
    this.emit(Opcode.STI, TypeCode.A);
    const exits: number[] = [];
    for (const routine of this.routineValues.filter(
      (routine) =>
        routine.parent.level === 0 &&
        !routine.methodOwner &&
        this.proceduralCompatible(target.type, this.proceduralType(routine))
    )) {
      this.loadVariable(value);
      this.literal(routine.proceduralId, INTEGER);
    ...
    const line = node.lineNumber ?? this.line;
    this.helper(`nil-procedural-call-${String(line)}`, 0, () => {
      throw new PascalError('Nil or invalid procedural variable', line);
    });
    for (const exit of exits) this.patch(exit);
  ...
  private methodTarget(
    node: CallNode
  ): { routine: Routine; receiver: Node; type: PascalType } | undefined {
    if (!node.receiver && !node.inherited) {
      for (const record of [...this.scope.withRecords].reverse()) {
        const routine = record.type.object?.methods.get(node.name.toLowerCase());
        if (routine) {
          if (routine.privateOwner && !this.inModule(routine.privateOwner))
            this.fail(node, `Private object method "${node.name}"`);
          return {
            routine,
            type: record.type,
            receiver: {
              ...
              pointer: {
                type: NodeType.IDENTIFIER,
                internalVariable: { ...record.pointer, type: { ...POINTER, base: record.type } },
              },
              exact: true,
            },
          };
    ...
    if (node.receiver) {
      ...
      if (
        ...
        this.lookup(String(target.name))?.kind === 'type'
      ) {
        for (let scope: Scope | null = this.scope; scope; scope = scope.parent) {
          if (scope.methodOwner && scope.self && this.descendsFrom(scope.methodOwner, type)) {
            ...
            return {
              routine,
              type,
              receiver: { type: NodeType.IDENTIFIER, internalVariable: scope.self },
            };
          }
        }
      ...
      if (scope.methodOwner && scope.self) {
        ...
        if (routine && type)
          return {
            routine,
            type,
            receiver: { type: NodeType.IDENTIFIER, name: 'Self', internalVariable: scope.self },
          };
      }
      const shadow = scope.symbols.get(node.name.toLowerCase());
      if (shadow && !(shadow.kind === 'variable' && shadow.result) && !node.inherited)
        return undefined;
    }
  ...
  private methodCall(
    node: CallNode,
    expression: boolean,
    method: { routine: Routine; receiver: Node; type: PascalType }
  ): PascalType {
    const bound = this.temp({ ...POINTER, base: method.type });
    this.addressVariable(bound);
    this.address(method.receiver);
    this.emit(Opcode.STI, TypeCode.A);
    ...
    if (!method.routine.virtual || node.inherited) {
      ...
      if (!expression && method.routine.declaration.routineKind === 'constructor')
        this.helper('discard-constructor-result', 1, () => undefined);
      return result;
    }
    const exits: number[] = [];
    ...
    const line = node.lineNumber ?? this.line;
    this.helper(`uninitialized-object-${String(line)}`, 0, () => {
      throw new PascalError('Object not initialized', line);
    });
    for (const exit of exits) this.patch(exit);
  ...
  private compileCall(node: CallNode, expression: boolean): PascalType {
    ...
    if (node.name.toLowerCase() === 'fail' && !node.receiver) {
      if (expression || node.arguments.length || !this.scope.constructorBody)
        this.fail(node, 'Fail is only valid inside a constructor');
      this.emit(Opcode.LDA, 0, 0);
      this.literal(false, BOOLEAN);
      this.emit(Opcode.STI, TypeCode.B);
      this.scope.exits.push(this.emit(Opcode.UJP));
      return VOID;
    }
    if (node.receiver) {
      const qualified = this.qualified({
        type: NodeType.FIELD_ACCESS,
        record: node.receiver,
        field: node.name,
        lineNumber: node.lineNumber,
      });
      if (qualified.type === NodeType.IDENTIFIER)
        return this.compileCall(
          { ...node, receiver: undefined, name: String(qualified.name) },
          expression
        );
    }
    ...
    if (shadow?.kind === 'type') {
      ...
      if (variableCast) {
        ...
        if (
          !(this.ordinal(target) && this.ordinal(from)) &&
          !(target.kind === 'pointer' && from.kind === 'pointer')
        ) {
          if (this.aggregate(target))
            this.fail(node, 'Array or record value requires an assignment or matching parameter');
          this.castView(variableCast);
      ...
      if (source.kind === 'pointer' && this.ordinal(target))
        this.emit(Opcode.CSP, 1, InternalProcedure.POINTER_BITS);
  ...
  private builtinReturn(builtin: BuiltinDef, arguments_: Node[]): PascalType {
    ...
    if (['abs', 'sqr', 'succ', 'pred'].includes(name) && arguments_[0]) {
      ...
      return type.kind === 'real' && this.coprocessorMode()
        ? this.exactReal(type)
          ? EXTENDED
          : INTERMEDIATE
        : type;
    ...
    if (
      builtin.procedureIndex >= 350 &&
      builtin.procedureIndex <= 370 &&
      builtin.returnType === TypeKind.POINTER
    )
      return { ...POINTER, base: CHAR };
    if (name === 'hi' || name === 'lo') return BYTE;
    // Segments and offsets are words.
    if (['seg', 'ofs', 'sptr', 'cseg', 'dseg', 'sseg'].includes(name)) return WORD;
    if (name === 'swap' && arguments_[0])
      return this.promoteInteger(this.expressionType(arguments_[0]));
  ...
  private builtin(node: CallNode, builtin: BuiltinDef, expression: boolean): PascalType {
    ...
    if (!expression && builtin.isFunction)
      return this.discardResult(node, this.builtinReturn(builtin, args), () =>
        this.builtin(node, builtin, true)
      );
    ...
    if (name === 'new' || name === 'dispose') {
      ...
      if (type.kind !== 'pointer' || !type.base || type.base.kind === 'void')
        this.fail(node, 'Typed pointer variable required');
      const methodNode = args[1];
      let methodCall: CallNode | undefined;
      if (methodNode) {
        if (!type.base.object || ![NodeType.IDENTIFIER, NodeType.CALL].includes(methodNode.type))
          this.fail(methodNode, 'Object constructor or destructor expected');
        const method = type.base.object.methods.get(String(methodNode.name).toLowerCase());
        if (
          !method ||
          method.declaration.routineKind !== (name === 'new' ? 'constructor' : 'destructor')
        )
      ...
      if (name === 'new' && methodCall) {
        ...
        const failed = this.emit(Opcode.FJP),
    ...
    if (name === 'high' || name === 'low') {
      ...
      if (type.kind === 'string') {
        if (name === 'high') this.stringCapacity(type);
        else this.literal(0, INTEGER);
        return INTEGER;
      }
    ...
    if (name === 'typeof') {
      ...
      if (!type.object.hasVirtual)
        this.fail(argument, 'TypeOf needs an object type with virtual methods');
    ...
    if (
      builtin.procedureIndex === 512 ||
      builtin.procedureIndex === 513 ||
      builtin.procedureIndex === 518
    ) {
    ...
    if (builtin.procedureIndex >= 450 && builtin.procedureIndex <= 456) {
      ...
      for (const [index, argument] of args.entries())
        this.requireType(
          argument,
          builtin.params[index]?.type === TypeKind.STRING ? STRING : INTEGER,
    ...
    if (
      ...
      for (const arg of args.slice(1)) {
        ...
        if (!((name === 'assign' || name === 'rename') && this.emitNullTerminated(arg)))
          this.expression(arg);
        count++;
      }
      if (name === 'assign') {
    ...
    if (builtin.procedureIndex === 331 || builtin.procedureIndex === 332)
      return this.interruptCall(node, builtin);
      ...
      if (parameter?.mode === ParamMode.VAR) {
        ...
        if (parameter.typeName) {
          const expected = this.unitType(
            this.modules.lookupProcedure(node.name)?.unit.toLowerCase() ?? '',
            parameter.typeName
          );
    ...
    if (['abs', 'sqr', 'succ', 'pred'].includes(name) && returnType.kind === 'integer') {
      ...
      if (this.exactReal(returnType) || (coprocessor && this.exactContext))
        this.helper(`8087x-sqr-${String(line)}`, 1, (value) =>
          extendedOperation('*', value as Real, value as Real, line)
        );
      else {
  ...
  private binaryLayout(type: PascalType): BinaryCell[] {
    ...
    return cells.map((cell) => ({
      ...this.binaryCell(cell.type),
      ...(gaps ? { offset: cell.offset } : {}),
    }));
  }
  ...
  private memoryAccess(node: Node): { type: PascalType; emit: () => void } | undefined {
    ...
    const access = node as ArrayAccessNode,
      array = this.qualified(access.array);
    ...
    if (
      port
        ? access.indices.length !== 1 || access.segmented
        : !access.segmented || access.indices.length !== 2
    )
      this.fail(
        node,
        port
          ? `${String(array.name)} takes one port number`
          : `${String(array.name)} takes a segment and offset, as [Seg:Ofs]`
      );
    return {
      type,
      emit: () => {
        for (const index of access.indices)
          this.requireType(index, INTEGER, this.expression(index));
        if (port) {
  ...
  private dataSegment(variables: Variable[]): SegmentLayout {
    const present = STANDARD_VARIABLES.filter(
      (standard) => !standard.unit || this.standardUnits.has(standard.unit)
    );
    const types = {
      word: WORD,
      longint: LONGINT,
    ...
    const standard = (names: readonly string[]) =>
      names
        .flatMap((name) => present.filter((entry) => entry.name === name))
        .map((entry) => ({
          offset: this.standardVariable(entry.name),
          type: types[entry.kind],
          aligned: true,
        }));
    return this.segmentLayout([
      ...
      ...this.typedConstants.map(({ variable }) => ({
        offset: variable.offset,
        type: variable.type,
        aligned: variable.aligned !== false,
      })),
  ...
  private frame(routine: Routine, parameters: Variable[]): SegmentLayout {
    ...
    for (const local of [...this.scope.locals].reverse())
      entries.push({ offset: local.offset, type: local.type, even: true });
    if (routine.type.kind !== 'void' && !this.scope.constructorBody)
      entries.push({ offset: 0, type: routine.type, even: true });
    entries.push({ gap: 4, even: true });
    if (this.scope.level > 1) entries.push({ gap: 2, even: true });
    if (this.scope.self)
      entries.push({ offset: this.scope.self.offset, type: POINTER, even: true });
    for (const parameter of [...parameters].reverse()) {
      ...
      entries.push({
        ...
        type: parameter.reference ? POINTER : parameter.type,
        even: true,
      });
    }
  ...
  private segmentLayout(
    ...
    return {
      ...
      refresh: this.refreshList(refreshes),
      variantCells,
      bytes: byte,
    };
  }
  ...
  private viewShape(type: PascalType): ViewShape {
    if (type.kind === 'array' && type.element) {
      ...
      return {
        kind: 'array',
        count: type.size / element.size,
        cells: element.size,
        bytes: element.byteSize,
        element: this.viewShape(element),
      };
    }
    if (type.kind === 'record' && type.fields) {
      ...
      const shadows = (type.variantParts ?? []).map(({ part, byteStart }) => {
        ...
        const shape: ViewShape = {
          ...
          cells: 1,
          bytes: 1,
          element: { kind: 'cell', cell: this.binaryCell(BYTE) },
        };
  ...
  private variantRanges(type: PascalType, offset: number): [number, number][] {
    if (type.kind === 'array' && type.element) {
      const element = type.element,
        inner = this.variantRanges(element, 0);
      if (!inner.length) return [];
      return Array.from({ length: type.size / element.size }, (_, index) =>
        inner.map(([from, to]): [number, number] => [
          from + offset + index * element.size,
          to + offset + index * element.size,
        ])
      ).flat();
    }
    ...
    return [...(type.fields?.values() ?? [])].flatMap((field) =>
      this.variantRanges(field.type, offset + field.offset)
    );
  }
  ...
  private variantRefreshes(type: PascalType, offset: number): { part: number; offset: number }[] {
    ...
    return (type.variantRefresh ?? []).map((entry) => ({
      part: entry.part,
  ...
  private stringMutation(node: CallNode, builtin: BuiltinDef): PascalType {
    ...
    if (name === 'delete') {
      ...
      this.emit(
        Opcode.CSP,
        type.kind === 'integer' ? 6 : type.kind === 'real' ? 5 : 4,
        builtin.procedureIndex
      );
    }
    return VOID;
  }
  ...
  private outputArgument(node: Node): void {
    ...
    const type = this.emitNullTerminated(value)
      ? STRING
      : this.inExactContext(this.coprocessorMode(), () => this.expression(value));
    ...
    this.helper(`format-${kind}-${String(line)}`, 3, (raw, rawWidth, rawPrecision) => {
      ...
      if (real)
        return formatReal(
          raw instanceof Float80 ? raw : Number(raw),
          width,
          precision,
          coprocessor
        );
