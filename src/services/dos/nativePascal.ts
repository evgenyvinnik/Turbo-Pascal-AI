...
export function nativePascalBatch(path: string, run: boolean, parameters = '', isUnit = false): string {
  const source = dosPath(path).replaceAll('/', '\\');
  if (!/\.(pas|pp)$/i.test(source)) throw new Error('Save the source with a .PAS or .PP extension first.');
  const name = source.split('\\').at(-1)!.replace(/\.(pas|pp)$/i, '');
  ...
  return [
    ...
    'goto end', ':failed', 'echo Compilation failed. The program was not run.', ':end', '',
  ].join('\r\n');
}
...
export async function openNativePascalSession(run = true): Promise<void> {
  ...
  if (/[\r\n\0]/.test(settings.programParameters)) throw new Error('Program parameters must be a single line.');
  const sources = { ...programDisk.snapshot() };
  for (const source of Object.values(useDesktopStore.getState().buffers)) sources[dosPath(source.path)] = bufferText(source);
  ...
  for (const [key, flag] of [['I', '-Ci'], ['Q', '-Co'], ['R', '-Cr']] as const) config.push(`${flag}${switches.find(([name]) => name === key)?.[1] ? '+' : '-'}`);
  for (const dir of sourceDirs) config.push(`-Fu"${dir}"`);
  for (const [field, flag] of [['unit', '-Fu'], ['include', '-Fi'], ['object', '-Fo']] as const) {
    for (const value of String(dirs?.[field] ?? '').split(';').filter(Boolean)) {
  ...
  for (const define of settings.defines.split(/[;,\s]+/).filter(Boolean)) {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(define)) throw new Error(`Invalid conditional define: ${define}`);
    config.push(`-d${define}`);
  }
  toolFiles['NATIVE.CFG'] = config.join('\r\n') + '\r\n';
  const sourceIsUnit = /^\s*(?:(?:\{[\s\S]*?\}|\(\*[\s\S]*?\*\))\s*)*unit\b/i.test(bufferText(buffer));
  toolFiles['NATIVE.BAT'] = nativePascalBatch(buffer.path, run && !sourceIsUnit, settings.programParameters, sourceIsUnit);
