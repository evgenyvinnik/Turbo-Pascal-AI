...
export const useDosStore = create<DosState>(() => ({
  visible: false,
  status: 'closed',
  error: null,
  transcript: '',
...
const fail = (error: unknown): void => {
  useDosStore.setState({ status: 'error', error: detail(error) });
};
...
export async function openDosSession(
  options: { command?: string; files?: DosFiles; nativePascal?: boolean; toolFiles?: DosFiles } = {}
): Promise<void> {
  ...
  useDosStore.setState({
    ...
    frameWidth: 0,
    frameHeight: 0,
    nativePascal: options.nativePascal ?? false,
  });
  closing = false;
  exited = false;
  stdoutTail = '';
  ...
  try {
    const ci = await startDosRuntime(
      initial,
      options.command,
      options.nativePascal,
      options.toolFiles
    );
    if (launch !== generation || !useDosStore.getState().visible) {
      await ci.exit();
      return;
    }
    machine = ci;
    ci.events().onFrameSize((width, height) => {
      useDosStore.setState({ width, height });
    });
    ...
    ci.events().onStdout((text) => {
      useDosStore.setState((state) => ({
        transcript: (state.transcript + text.replaceAll(SIGNAL_LINE, '') + '\n').slice(-32768),
      }));
      ...
      if (stdoutTail.includes(DOS_EXIT_SIGNAL)) {
        stdoutTail = '';
        void closeDosSession();
      }
    });
    ...
    try {
      ...
      useDosStore.setState({
        frame: new Uint8ClampedArray(screenshot.data),
        width: screenshot.width,
        height: screenshot.height,
        frameWidth: screenshot.width,
        frameHeight: screenshot.height,
      });
    } catch {
...
function commitDosFiles(result: DosFiles): number {
  ...
  for (const name of diskChanges.keys()) {
    if (current[name] !== diskAtLaunch[name] && current[name] !== result[name])
      throw new Error(
        `${name} changed in both DOS and the browser. Export DOS files to keep both versions.`
      );
  }
  const desktop = useDesktopStore.getState();
  for (const buffer of Object.values(desktop.buffers)) {
    if (
      diskChanges.has(dosPath(buffer.path)) &&
      editorAtLaunch.get(buffer.id) !== bufferText(buffer)
    )
      throw new Error(
        `${buffer.name} has unsaved browser edits. Export DOS files to keep both versions.`
      );
  }
  ...
  if (diskChanges.size) {
    ...
    for (const buffer of Object.values(buffers)) {
      ...
      if (changed === null) {
        buffers[buffer.id] = { ...buffer, modified: true };
        continue;
      }
      ...
      buffers[buffer.id] = {
        ...
        error: null,
        highlight: null,
        undo: [...buffer.undo, { lines: buffer.lines, cursor: buffer.cursor }],
        redo: [],
      };
...
export function sendDosCommand(command: string): Promise<void> {
  return queued(async () => {
    if (machine) await typeDosCommand(machine, command);
  });
}
...
export function openDosDebugger(): Promise<void> {
  return queued(async () => {
    ...
    if (useDosStore.getState().nativePascal) {
      let target: string;
      try {
        target = bytesToString(await machine.fsReadFile('__TPTOOLS/BUILD.OK')).trim();
      } catch {
        throw new Error(
          'Run a successfully compiled Pascal program before opening its CPU debugger.'
        );
      }
...
export async function exportDosFiles(): Promise<Uint8Array> {
  return queued(async () => {
    ...
    return zipSync(
      Object.fromEntries(
        Object.entries(files).map(([path, content]) => [path, stringToBytes(content)])
      )
    );
  });
}
...
export async function importFilesToDos(files: readonly File[]): Promise<void> {
  return queued(async () => {
    ...
    for (const file of files) {
      if (file.size > programDisk.capacity)
        throw new Error('Selected file exceeds the 8 MiB drive capacity.');
      const bytes = new Uint8Array(await file.arrayBuffer());
      const entries = /\.(zip|jsdos)$/i.test(file.name)
        ? importDosArchive(bytes)
        : { [dosPath(file.webkitRelativePath || file.name)]: bytesToString(bytes) };
      for (const [name, content] of Object.entries(entries)) {
        if (additions[name] !== undefined && additions[name] !== content)
    ...
    for (const [name, content] of Object.entries(additions))
      if (current[name] !== undefined && current[name] !== content)
        throw new Error(`${name} already exists. Rename the imported file to keep both copies.`);
    reconcileDosFiles(current, current, { ...current, ...additions });
    for (const [name, content] of Object.entries(additions))
      await machine.fsWriteFile(`USER/${name}`, stringToBytes(content));
