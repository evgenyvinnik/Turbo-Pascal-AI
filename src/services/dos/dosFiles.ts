...
export function reconcileDosFiles(
  ...
  for (const name of new Set([...Object.keys(initial), ...Object.keys(result)])) {
    ...
    if (current[name] !== initial[name] && current[name] !== result[name])
      throw new Error(
        `${name} changed in both DOS and the browser. Export the DOS files before returning.`
      );
  ...
  if (Object.values(candidate).reduce((n, contents) => n + contents.length, 0) > DOS_CAPACITY)
    throw new Error(
      'The DOS files exceed the browser drive’s 8 MiB capacity. Export the DOS files before returning.'
    );
  return changes;
}
...
export function importDosArchive(bytes: Uint8Array): DosFiles {
  ...
  const archive = unzipSync(bytes, {
    filter(entry) {
      ...
      if (size > DOS_CAPACITY)
        throw new Error('The expanded ZIP exceeds the browser drive’s 8 MiB capacity.');
      return true;
    },
  });
  return Object.fromEntries(
    Object.entries(archive).map(([path, content]) => [dosPath(path), bytesToString(content)])
  );
}
