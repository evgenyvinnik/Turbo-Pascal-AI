...
export function DosWorkspace() {
  ...
  useEffect(() => {
    ...
    canvas.current
      .getContext('2d')
      ?.putImageData(new ImageData(new Uint8ClampedArray(frame), frameWidth, frameHeight), 0, 0);
  ...
  useEffect(() => {
    ...
    const release = () => {
      for (const code of held) getDosMachine()?.sendKeyEvent(code, false);
      held.clear();
    };
    const onKey = (event: KeyboardEvent) => {
      ...
      if (code < 340) {
        const shiftedCharacter =
          event.key.length === 1 &&
          ('~!@#$%^&*()_+{}|:"<>?'.includes(event.key) || /[A-Z]/.test(event.key));
        for (const [modifier, active] of [
          [340, event.shiftKey || shiftedCharacter],
          [341, event.ctrlKey],
          [342, event.altKey],
        ] as const) {
          getDosMachine()?.sendKeyEvent(modifier, active);
          if (active) held.add(modifier);
  ...
  return (
    <section
      className="dos-workspace"
      aria-label="DOS workspace"
      onDrop={(event) => {
        event.preventDefault();
    ...
    >
      <div className="dos-toolbar">
        ...
        <button onClick={() => void closeDosSession()} disabled={busy}>
          Return to IDE
        </button>
        <button
          disabled={!ready}
          onClick={() => void act(syncDosFiles, 'DOS files saved to the browser drive.')}
        >
          Save files
        </button>
        ...
        <button
          ...
          onClick={() =>
            void act(async () => {
              ...
              anchor.href = url;
              anchor.download = 'DOS-WORKSPACE.zip';
              anchor.click();
              setTimeout(() => {
        ...
        <button
          ...
          onClick={() =>
            void act(
              openDosDebugger,
              'DEBUG: R registers, U disassembly, D memory, A assemble, T step, P step over, G run, Q quit.'
            )
          }
        >
          CPU debugger
        </button>
        <button disabled={!ready} onClick={() => void act(() => openNativePascalSession(true))}>
          Run Pascal
        </button>
        <button
          disabled={!ready}
          onClick={() =>
            void act(
              () =>
                importFilesToDos(
                  Object.entries(DOS_EXAMPLES).map(
                    ([name, data]) => new File([new Uint8Array(stringToBytes(data))], name)
                  )
                ),
              'Try HELLO, or DEBUG < X86TEST.SCR to assemble and trace REGTEST.COM.'
            )
          }
        >
          Examples
        ...
        {state.error && (
          <button onClick={() => void discardDosSession()}>Discard DOS changes</button>
        )}
        <input
          ref={input}
          type="file"
      ...
      <div className="dos-display">
        <canvas
          ref={canvas}
          width={frameWidth || state.width}
          height={frameHeight || state.height}
          tabIndex={0}
          aria-label="DOS screen"
          onMouseMove={(event) => {
            const rect = event.currentTarget.getBoundingClientRect();
            getDosMachine()?.sendMouseMotion(
          ...
          onMouseDown={(event) => {
            canvas.current?.focus();
            getDosMachine()?.sendMouseButton(event.button, true);
          }}
          onMouseUp={(event) => getDosMachine()?.sendMouseButton(event.button, false)}
          onContextMenu={(event) => {
            event.preventDefault();
          }}
        />
      ...
      <div className="dos-status" role="status">
        {state.error ??
          (notice ||
            'EXIT or Ctrl+Alt+Esc returns to the IDE and saves files. Import your DOS programs to run them here.')}
      </div>
      <pre className="dos-transcript" aria-label="DOS output">
        {state.transcript}
      </pre>
    </section>
  );
