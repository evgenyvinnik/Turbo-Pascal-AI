...
} from '@services/db/workspaceRepository';
import {
  decodeWorkspace,
...
function captureWorkspace(): WorkspaceSnapshot {
  ...
  return {
    ...
    ide: {
      helpTopic: ide.helpTopic,
      search: ide.search,
      destination: ide.destination,
      primaryFile: ide.primaryFile,
      optionsFile: ide.optionsFile,
      directory: ide.directory,
      compilerOptions: ide.compilerOptions,
      optionDialogs: ide.optionDialogs,
      tools: ide.tools,
      programParameters: ide.programParameters,
      defines: ide.defines,
    },
    debug: {
...
function applyWorkspace(snapshot: WorkspaceSnapshot): void {
  useDesktopStore.setState({
    ...snapshot.desktop,
    buffers: Object.fromEntries(
      Object.entries(snapshot.desktop.buffers).map(([id, buffer]) => [
        id,
        { ...buffer, error: null, highlight: null },
      ])
    ),
  });
  ...
  useDebugStore.setState({
    ...
    watches: snapshot.debug.watches.map((watch) => ({
      ...watch,
      value: undefined,
      type: 'unknown',
    })),
...
async function flushWorkspace(): Promise<void> {
  ...
  try {
    ...
    const record = await saveWorkspace(captureWorkspace(), {
      expectedRevision: revision,
      preserveConflict: resolveConflict,
    });
    revision = record.revision;
    succeeded = true;
    if (!isDisposed())
      useWorkspaceStore.setState({
        status: hasPendingSave() ? 'saving' : 'saved',
        lastSavedAt: record.updatedAt,
        error: null,
        ...(record.recoveryId
          ? { notice: 'This tab is saved; the other workspace is kept as a recovery copy.' }
          : {}),
      });
  } catch (error) {
    dirty = true;
    if (!isDisposed())
      useWorkspaceStore.setState({
        ...
        conflict: error instanceof WorkspaceConflictError,
      });
  } finally {
    writing = false;
...
function subscribeToWorkspace(): void {
  unsubscribers.push(
    useDesktopStore.subscribe((next, previous) => {
      const changed =
        next.windows !== previous.windows ||
        next.activeId !== previous.activeId ||
        next.clipboard !== previous.clipboard ||
        next.seq !== previous.seq ||
        next.untitled !== previous.untitled ||
  ...
  unsubscribers.push(
    useIdeStore.subscribe((next, previous) => {
      if (ideKeys.some((key) => next[key] !== previous[key])) scheduleWorkspaceSave();
    })
  );
  unsubscribers.push(
    useDebugStore.subscribe((next, previous) => {
      if (
        next.breakpoints !== previous.breakpoints ||
        next.watches.length !== previous.watches.length ||
  ...
  unsubscribers.push(
    usePopupStore.subscribe((next, previous) => {
      if (next.histories !== previous.histories) scheduleWorkspaceSave();
    })
  );
  unsubscribers.push(
    useCompilerStore.subscribe((next, previous) => {
      if (next.programOutput !== previous.programOutput || next.messages !== previous.messages)
        scheduleWorkspaceSave();
    })
  );
  ...
  try {
    ...
    if (record) {
      ...
      try {
        ...
        useWorkspaceStore.setState({
          notice: 'Started a fresh workspace; a recovery copy is kept in IndexedDB.',
        });
      }
      if (snapshot) {
        applyWorkspace(snapshot);
    ...
    useWorkspaceStore.setState({
      ready: true,
      status: 'saved',
      lastSavedAt: restored ? (record?.updatedAt ?? null) : null,
    });
