...
async function grepSources(argumentsText: string): Promise<void> {
  ...
  for (const name of [...new Set([...SAMPLE_FILES, ...virtualFiles()])])
    sources[name] = await loadSample(name);
...
function goToMessage(direction: -1 | 0 | 1): void {
  ...
  for (let attempt = 0; attempt < messages.length; attempt++) {
    if (direction || attempt)
      index = (index + (direction || 1) + messages.length) % messages.length;
...
const configuration = () => {
  ...
  return {
    ...
    optionDialogs: state.optionDialogs,
    defines: state.defines,
    tools: state.tools,
    programParameters: state.programParameters,
...
function loadConfiguration(name: string): void {
  ...
  if (
    ...
    !['Memory', 'Disk'].includes(String(value.destination)) ||
    ['defines', 'programParameters', 'primaryFile', 'directory'].some(
  ...
  useIdeStore.setState({
    ...
    optionDialogs: settings.optionDialogs,
    defines: settings.defines,
    tools: settings.tools,
    programParameters: settings.programParameters,
    destination: settings.destination,
    primaryFile: settings.primaryFile,
    directory: settings.directory,
  });
  useIdeStore.setState({ optionsFile: name });
}

const message = (title: string, lines: string[]) => {
  dlg().open(D.messageDialog(title, lines));
};
...
const fileListing = (): { files: string[]; info: string[] } => {
  ...
  const files = [
    ...new Set([
      ...SAMPLE_FILES,
      ...open,
      ...virtualFiles().filter((name) => /\.pas$/i.test(name)),
    ]),
  ].sort();
  return {
    files,
...
function textOffset(buffer: Buffer, position: Pos): number {
  return (
    buffer.lines.slice(0, position.line).reduce((length, line) => length + line.length + 1, 0) +
    position.col
  );
}
...
function beginSearch(buffer: Buffer): void {
  ...
  if (ide().search.selectedOnly) {
    ...
    searchScope = {
      bufferId: buffer.id,
      start: textOffset(buffer, start),
      end: textOffset(buffer, end),
    };
  }
}
...
function textMatches(buffer: Buffer): TextMatch[] {
  ...
  try {
    expression = new RegExp(
      searchPattern(settings.text, settings.regularExpression),
      settings.caseSensitive ? 'gm' : 'gim'
    );
  } catch {
    throw new Error('Invalid regular expression');
  }
  ...
  for (const match of source.matchAll(expression)) {
    ...
    if (
      settings.wholeWords &&
      (/[A-Za-z0-9_]/.test(source[start - 1] ?? '') || /[A-Za-z0-9_]/.test(source[end] ?? ''))
    )
      continue;
...
function showSearchMatch(buffer: Buffer, match: TextMatch): void {
  ...
  desk().ensureVisible(
    Math.max(1, (window?.rect.h ?? 23) - 2),
    Math.max(1, (window?.rect.w ?? 80) - 2)
  );
...
function searchAgain(initial = false): void {
  ...
  try {
    ...
    const previous =
      !initial &&
      lastSearch?.bufferId === buffer.id &&
      lastSearch.source === buffer.lines.join('\n') &&
      textOffset(buffer, buffer.cursor) === lastSearch.match.end &&
      buffer.anchor !== null &&
      textOffset(buffer, buffer.anchor) === lastSearch.match.start
        ? lastSearch.match
        : null;
    const cursor = textOffset(buffer, buffer.cursor);
    const from =
      initial && settings.entireScope
        ? settings.backward
          ? Infinity
          : -1
        : previous
          ? settings.backward
            ? previous.start - 1
            : Math.max(previous.end, previous.start + 1)
          : cursor;
    const ordered = settings.backward ? [...matches].reverse() : matches;
    const match =
      ordered.find((candidate) =>
        settings.backward ? candidate.start <= from : candidate.start >= from
      ) ?? ordered[0];
...
function replaceMatches(bufferId: string, matches: TextMatch[], recordUndo = true): void {
  ...
  for (const match of [...matches].sort((a, b) => b.start - a.start))
    text = text.slice(0, match.start) + settings.replacement + text.slice(match.end);
  desk().edit((next) => {
    if (recordUndo) {
...
function replaceText(all: boolean): void {
  ...
  try {
    const settings = ide().search;
    const cursor = textOffset(buffer, buffer.cursor);
    let matches = textMatches(buffer).filter(
      (match) =>
        settings.entireScope || (settings.backward ? match.end <= cursor : match.start >= cursor)
    );
    ...
    const complete = (): void => {
      message('Information', [
        `${String(accepted.length)} occurrence${accepted.length === 1 ? '' : 's'} replaced`,
      ]);
    };
    if (!settings.promptOnReplace) {
      accepted.push(...matches);
      replaceMatches(buffer.id, accepted);
      complete();
      return;
    }
    let index = 0;
        ...
        (action) => {
          if (action === 'yes') {
            ...
            if (!settings.backward)
              offset += settings.replacement.length - (match.end - match.start);
          }
...
async function doCompile(
  ...
  let buf =
    (ide().primaryFile
      ? Object.values(desktop.buffers).find((buffer) => buffer.name === ide().primaryFile)
      : undefined) ??
    desktop.activeBuffer() ??
    (sourceWindow?.bufferId ? desktop.buffers[sourceWindow.bufferId] : null);
  ...
  const result = await useCompilerStore.getState().compile(source, buf.path, {
    ...
    defines: ide()
      .defines.split(/[;,\s]+/)
      .filter(Boolean),
    includeDirectories: String(directories?.include ?? '')
      .split(';')
      .filter(Boolean),
    unitDirectories: String(directories?.unit ?? '')
      .split(';')
      .filter(Boolean),
  });
  if (request !== compileRequest) return;
  ...
  if (result.nativeRequired) {
    await openNativePascalSession(thenRun && debugAction === 'run').catch((error: unknown) => {
      message('Native Pascal', [error instanceof Error ? error.message : String(error)]);
    });
    return;
  }
  ...
  if (result.errors.length > 0) {
    ...
    const errorBuffer = Object.values(desk().buffers).find(
      (buffer) => sourcePath(buffer.path) === sourcePath(err?.file ?? buf.name)
    );
    const errorSource = Object.entries(sources).find(
      ([name]) => sourcePath(name) === sourcePath(err?.file ?? '')
    )?.[1];
    if (!errorBuffer && err?.file && errorSource !== undefined)
      desk().openFile(err.file, err.file, errorSource);
    const window = desk().windows.find(
      (win) => win.bufferId === (errorBuffer?.id ?? desk().activeBuffer()?.id ?? buf.id)
    );
  ...
  dlg().open(
    ...
    (action) => {
      if (thenRun && action === 'ok' && request === compileRequest && result.bytecode) {
        if (result.isUnit)
          message('Error', ['Cannot run a unit.', 'Open a program that uses this unit.']);
  ...
  dlg().open(D.evaluateDialog(), values, (action, updated) => {
    ...
    try {
      const result = evaluateProgram(
        String(updated.expr ?? ''),
        action === 'modify' ? String(updated.value ?? '') : undefined
      );
...
function editBreakpoint(id?: string, after?: () => void): void {
  ...
  if (!file) {
    message('Breakpoint', ['Open a source file first']);
    return;
  }
  dlg().open(
    {
      id: 'edit-breakpoint',
      title: existing ? 'Edit Breakpoint' : 'Add Breakpoint',
      ...
      controls: [
        { kind: 'label', x: 3, y: 2, text: '~C~ondition', for: 'condition' },
        {
          ...
          y: 3,
          w: 30,
          hint: 'Enter expression which must be TRUE for breakpoint to take effect',
        },
        { kind: 'label', x: 3, y: 5, text: '~P~ass count', for: 'passes' },
        {
          ...
          w: 30,
          history: false,
          hint: 'Number of matching passes to skip before pausing',
        },
        { kind: 'label', x: 3, y: 8, text: '~F~ile name', for: 'file' },
        {
          ...
          w: 30,
          history: false,
          hint: 'Enter the source file containing this breakpoint',
        },
        { kind: 'label', x: 3, y: 11, text: '~L~ine number', for: 'line' },
        {
          ...
          w: 30,
          history: false,
          hint: 'Enter the source line where execution should pause',
        },
        {
          kind: 'button',
          id: 'ok',
        ...
        },
        {
          ...
          label: 'Cancel',
          result: 'cancel',
          hint: 'Close without changing the breakpoint',
        },
        {
          kind: 'button',
          id: 'help',
          ...
          label: 'Help',
          result: 'help',
          hint: 'Show help about breakpoints',
        },
      ],
    },
    {
      line: String(existing?.line ?? (buffer?.cursor.line ?? 0) + 1),
      condition: existing?.condition ?? '',
      passes: String(existing?.passCount ?? 0),
      file,
    },
    (action, values) => {
      if (action === 'ok') {
        ...
        if (!Number.isSafeInteger(line) || line < 1) {
          message('Breakpoint', ['Enter a positive line number']);
          return;
        }
        const debug = useDebugStore.getState();
        const passes = Number(values.passes);
        if (!Number.isSafeInteger(passes) || passes < 0) {
          message('Breakpoint', ['Enter a nonnegative pass count']);
          return;
        }
        const targetFile = String(values.file ?? file)
          .trim()
          .toUpperCase();
        if (existing) debug.removeBreakpoint(existing.id);
        debug.addBreakpoint(targetFile, line);
        debug.updateBreakpoint(`bp-${targetFile}-${String(line)}`, {
...
function showBreakpoints(): void {
  ...
  dlg().open(
    D.breakpointsDialog(
      entries.map(
        (point) =>
          `${point.file.padEnd(24)}${String(point.line).padStart(6)} ${(point.enabled ? (point.condition ?? '') : '[off]').padEnd(31)}${String(point.passCount ?? 0).padStart(4)}`
      )
    ),
    {},
    (action, values) => {
      ...
      if (action === 'delete' && selected) {
        useDebugStore.getState().removeBreakpoint(selected.id);
        showBreakpoints();
      }
      if (action === 'clear') {
        for (const entry of entries) useDebugStore.getState().removeBreakpoint(entry.id);
        showBreakpoints();
      }
      if (action === 'edit') editBreakpoint(selected?.id, showBreakpoints);
      if (action === 'view' && selected) {
        const source = desk().windows.find(
          (win) => win.bufferId && desk().buffers[win.bufferId]?.name === selected.file
        );
        if (source) {
          desk().focusWindow(source.id);
          desk().gotoLine(selected.line);
        }
      }
    }
  );
...
export function runCommand(id: string): void {
  ...
  if (configuredTool) {
    ...
    if (!tool) return;
    if (/(?:^|[\\/])GREP\.(?:COM|EXE)$/i.test(tool.program) || tool.title.toLowerCase() === 'grep')
      id = 'tools.grep';
    else {
      stopProgram(false);
      void openDosSession({ command: `${tool.program} ${tool.params}`.trim() }).catch(
        (error: unknown) => {
          message(tool.title, [error instanceof Error ? error.message : String(error)]);
        }
      );
      return;
    }
  ...
  switch (id) {
    ...
    case 'file.open': {
      ...
      dlg().open(D.openFileDialog(files, info), { name: '*.PAS' }, (result, values) => {
        ...
        const typed = String(values.name ?? '')
          .trim()
          .toUpperCase();
        const chosen =
          typed && !typed.includes('*') ? typed : (files[Number(values.files ?? 0)] ?? '');
    ...
    case 'file.chdir':
      dlg().open(
        ...
        { dir: ide().directory },
        (result, values) => {
          if (result === 'ok' || result === 'chdir')
            ide().setDirectory(String(values.dir ?? 'A:\\'));
        }
      );
      return;
    ...
    case 'file.print': {
      ...
      if (buffer)
        printSource(
          buffer.name,
          bufferText(buffer),
          ((ide().optionDialogs['file.printer']?.highlight ?? [true]) as boolean[])[0] ?? true
        );
      return;
    }
    ...
    case 'file.printer':
      dlg().open(
        ...
        ide().optionDialogs['file.printer'] ?? {
          filter: 'PRNFLTR',
          command: '$NOSWAP /EPSON',
          highlight: [true],
        },
        (result, values) => {
          ...
          if (result === 'help')
            message('Printer Setup', [
              'Configure source highlighting here.',
              'File > Print opens the browser',
              'print dialog. DOS filter settings',
    ...
    case 'file.dos':
      stopProgram(false);
      void openDosSession().catch((error: unknown) => {
        message('DOS shell', [error instanceof Error ? error.message : String(error)]);
      });
      return;
      ...
      dlg().open(
        D.findDialog(),
        {
          ...
          options: [
            ...
            ide().search.wholeWords,
            ide().search.regularExpression,
          ],
    ...
    case 'search.goto': {
      ...
      dlg().open(
        D.gotoLineDialog(),
        { line: String((buf?.cursor.line ?? 0) + 1) },
        (result, values) => {
          if (result !== 'ok') return;
    ...
    case 'search.finderror':
      dlg().open(D.findErrorDialog(), { address: '0000:0000' }, (action, values) => {
        ...
        const pc =
          parts.length === 2
            ? parseInt(parts[0] ?? '', 16) * 16 + parseInt(parts[1] ?? '', 16)
            : parseInt(address, 16);
        const compiler = useCompilerStore.getState();
        const line = compiler.result?.bytecode?.sourceLines[pc];
        const source = desk().windows.find(
          (win) => win.bufferId && desk().buffers[win.bufferId]?.name === compiler.currentFile
        );
        if (line && source) {
          desk().focusWindow(source.id);
          desk().gotoLine(line);
    ...
    case 'search.findproc':
      dlg().open(D.findProcedureDialog(), { name: searchText() }, (action, values) => {
        ...
        const declaration = new RegExp(
          `\\b(?:procedure|function)\\s+${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`,
          'i'
        );
        for (const buffer of Object.values(desk().buffers)) {
    ...
    case 'run.params':
      dlg().open(
        D.programParametersDialog(),
        { params: ide().programParameters },
        (result, values) => {
    ...
    case 'compile.primary': {
      const { files, info } = fileListing();
      dlg().open(
        D.primaryFileDialog(files, info),
        { name: ide().primaryFile || '*.PAS' },
        (result, values) => {
    ...
    case 'tools.grep': {
      const configured =
        ide().tools.find(
          (tool) =>
            /(?:^|[\\/])GREP\.(?:COM|EXE)$/i.test(tool.program) ||
            tool.title.toLowerCase() === 'grep'
        )?.params ?? '';
      if (configured !== grepConfiguration) {
        grepConfiguration = configured;
        grepArguments = '';
      }
      dlg().open(
        programArgumentsDialog(),
        { args: grepArguments || configured || `${searchText() || 'program'} *.pas` },
        (result, values) => {
          if (result !== 'ok') return;
          grepArguments = String(values.args ?? '');
          void grepSources(grepArguments).catch((error: unknown) => {
            message('Grep', [error instanceof Error ? error.message : String(error)]);
          });
        }
      );
      return;
    ...
    case 'debug.togglebreak': {
      ...
      const existing = [...useDebugStore.getState().breakpoints.values()].find(
        (point) => point.file === buffer.name && point.line === line
      );
    ...
    case 'debug.gotosource': {
      ...
      const source = d.windows.find(
        (win) => win.bufferId && d.buffers[win.bufferId]?.name === frame?.file
      );
      if (source && frame) {
        d.focusWindow(source.id);
        d.gotoLine(frame.line);
      }
      return;
    }
    ...
    case 'options.saveas': {
      ...
      dlg().open(
        D.optionsFileDialog(id === 'options.saveas', files),
        { name: ide().optionsFile },
        (result, values) => {
          if (result !== 'ok') return;
    ...
    case 'options.save':
      try {
        saveConfiguration(ide().optionsFile);
      } catch (error) {
        message('Error', [error instanceof Error ? error.message : String(error)]);
      }
      return;
            ...
            dlg().open(
              {
                ...
                controls: [
                  { kind: 'label', x: 3, y: 2, text: '~T~itle', for: 'title' },
                  {
                    ...
                    y: 3,
                    w: 51,
                    hint: 'Enter the title displayed in the Tools menu',
                  },
                  { kind: 'label', x: 3, y: 5, text: '~P~rogram path', for: 'program' },
                  {
                    ...
                    y: 6,
                    w: 51,
                    hint: 'Enter the path of the tool program',
                  },
                  { kind: 'label', x: 3, y: 8, text: 'P~a~rameters', for: 'params' },
                  {
                    ...
                    y: 9,
                    w: 51,
                    hint: 'Enter command line parameters for the tool',
                  },
                  {
                    kind: 'button',
                    id: 'ok',
                    ...
                    result: 'ok',
                    default: true,
                    hint: 'Accept the settings in this dialog box',
                  },
                  {
                    kind: 'button',
                    id: 'cancel',
                    ...
                    label: 'Cancel',
                    result: 'cancel',
                    hint: 'Close the dialog box without making any changes',
                  },
                  {
                    kind: 'button',
                    id: 'help',
                    ...
                    label: 'Help',
                    result: 'help',
                    hint: 'Show help about using this dialog box',
                  },
                ],
              },
              entry ? { ...entry } : {},
    ...
    case 'window.list': {
      const showWindows = (selection?: number): void => {
        const windows = [...desk().windows].sort(
          (a, b) => a.title.localeCompare(b.title) || a.num - b.num
        );
        ...
        dlg().open(
          D.windowListDialog(windows.map((win) => win.title)),
          { windows: selected },
          (result, values) => {
    ...
    case 'help.context':
      {
        const context =
          dlg().top()?.def.id ??
          (useMenuStore.getState().open ? 'menus' : (d.activeWindow()?.kind ?? 'edit'));
...
export const helpLines = (topic: string): string[] => getHelpLines(topic);
