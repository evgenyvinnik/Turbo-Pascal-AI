...
import { SourceDebugger, type DebugAction, type DebugValue } from '@compiler/runtime/SourceDebugger';
...
function prepareSound(): void {
  ...
  } catch { /* Audio is optional when the browser does not offer an audio device. */ }
}

function sound(frequency: number): void {
  if (!audio) return;
  if (frequency <= 0) { oscillator?.stop(); oscillator = null; return; }
  if (!oscillator) {
...
function clearDebug(): void {
  ...
  useDesktopStore.setState({ buffers: Object.fromEntries(Object.entries(desktop.buffers).map(([id, buffer]) => [id,
...
export function refreshDebug(): void {
  ...
  for (const watch of useDebugStore.getState().watches) {
    try { values[watch.expression] = session.debugger.evaluate(watch.expression); }
    catch (error) { values[watch.expression] = { value: error instanceof Error ? error.message : String(error), type: 'error' }; }
  }
  useDebugStore.getState().updateSnapshot({
    status: session.debugger.isPaused() ? 'paused' : 'running', file: session.debugger.getFile() ?? session.file,
    line: session.debugger.getLine(), registers: session.machine.getRegisters(), values,
    frames: session.debugger.frames().map((frame) => ({
      id: String(frame.mp), name: frame.name, file: frame.file ?? session.file, line: frame.line,
      label: frame.arguments.length ? `${frame.name}(${frame.arguments.map((value) => typeof value === 'string' ? `'${decodeDosText(value)}'` : String(value)).join(',')})` : frame.name,
...
export function evaluateProgram(expression: string, replacement?: string): DebugValue {
  if (!current?.debugger.isPaused()) throw new Error('Pause the program to inspect or modify values');
  const value = replacement === undefined ? current.debugger.evaluate(expression) : current.debugger.modify(expression, replacement);
  refreshDebug();
  return value;
}
...
export function resumeProgram(action: DebugAction, bufferId?: string, targetLine = 0): boolean {
  ...
  if (targetBuffer && bufferId !== current.bufferId && current.sources[sourcePath(targetBuffer.path)] === undefined) return false;
  const buffer = useDesktopStore.getState().buffers[current.bufferId];
  if (!buffer || bufferText(buffer) !== current.source) { stopProgram(false); return false; }
  if (Object.values(useDesktopStore.getState().buffers).some(candidate => {
    const original = current!.sources[sourcePath(candidate.path)];
    return original !== undefined && original !== bufferText(candidate);
  })) { stopProgram(false); return false; }
  if (current.machine.getState() === MachineState.WAITING) {
    if (current.machine.getConsole().active || current.machine.getGraphics().initialized) useProgramScreenStore.setState({ visible: true });
    return true;
  }
...
function focusSource(session: Session, file: string): void {
  const desktop = useDesktopStore.getState();
  const buffer = Object.values(desktop.buffers).find(candidate => sourcePath(candidate.path) === sourcePath(file));
...
function inputDialog(output: string[]): DialogDef {
  const prompt = [...output].reverse().find((line) => line.trim()) ?? 'Enter a line of input, or press Enter to continue.';
  return {
    id: 'program-input',
    title: 'Program input',
    ...
    controls: [
      ...
      { kind: 'input', id: 'line', x: 3, y: 4, w: 64, history: false, hint: 'Enter program input; Enter submits, Esc or Ctrl+F2 stops' },
      { kind: 'button', id: 'ok', x: 22, y: 7, w: 10, label: '~E~nter', result: 'ok', default: true, hint: 'Send this line to the running program' },
      { kind: 'button', id: 'cancel', x: 38, y: 7, w: 10, label: '~S~top', result: 'cancel', hint: 'Stop the running program' },
    ],
  };
}
...
function fail(session: Session, error: unknown): void {
  ...
  compiler.setMessages([...compiler.messages, `${file}(${String(line)}): ${text}`, ...(detail === diagnostic.message ? [] : [`  ${detail}`])]);
  const desktop = useDesktopStore.getState();
  ...
  useDialogStore.getState().open(messageDialog('Runtime error', [
...
function programKey(event: KeyboardEvent): string | null {
  ...
  const scans: Record<string, number> = {
    Home: 71, ArrowUp: 72, PageUp: 73, ArrowLeft: 75, ArrowRight: 77,
  ...
  if (functionKey) {
    ...
    const first = number <= 10 ? (event.altKey ? 104 : event.ctrlKey ? 94 : event.shiftKey ? 84 : 59) : (event.altKey ? 139 : event.ctrlKey ? 137 : event.shiftKey ? 135 : 133);
  ...
  const controls: Record<string, string> = { Enter: '\r', Escape: '\x1b', Backspace: '\b', Tab: '\t' };
  if (controls[event.key]) return controls[event.key] ?? null;
  if (event.ctrlKey) return /^[a-z]$/i.test(event.key) ? String.fromCharCode(event.key.toUpperCase().charCodeAt(0) - 64) : null;
...
export function handleProgramScreenKey(event: KeyboardEvent): boolean {
  ...
  if (event.altKey && event.key === 'F5') { screen.hide(); return true; }
  const session = current;
  if (!session) { if (event.key === 'Escape') screen.hide(); return true; }
  ...
  if (event.key === 'Escape') { screen.hide(); return true; }
  if (event.key === 'Enter') {
    try {
  ...
  } else if (event.key === 'Backspace') useProgramScreenStore.setState({ input: screen.input.slice(0, -1) });
  else if (event.key.length === 1 && !event.ctrlKey && !event.altKey) useProgramScreenStore.setState({ input: screen.input + event.key });
  return true;
}

export function toggleProgramScreen(): void {
  const screen = useProgramScreenStore.getState();
  if (screen.console || screen.graphics) useProgramScreenStore.setState({ visible: !screen.visible });
  else useDesktopStore.getState().toggleTool('output');
}

export function startProgram(bytecode: Bytecode, file: string, bufferId: string, action: DebugAction = 'run', targetLine = 0): void {
  stopProgram(false);
  prepareSound();
  useProgramScreenStore.setState({ visible: false, console: null, graphics: null, waiting: false, input: '', kind: 'text' });
  const machine = new Machine(bytecode, { maxInstructions: 5_000_000, fileSystem: disk, onSound: sound });
  machine.reset();
  ...
  const sources = Object.fromEntries(Object.entries(bytecode.sources).map(([name, text]) => [sourcePath(name), text]));
  const session: Session = { machine, file, bufferId, timer: null, debugger: debuggerSession, source: source ? bufferText(source) : '', sources };
  current = session;
...
if (import.meta.hot) import.meta.hot.dispose(() => { stopProgram(false); });
