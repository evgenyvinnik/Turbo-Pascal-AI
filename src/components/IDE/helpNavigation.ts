...
const CONTEXTS: Record<string, string> = {
  open: 'Open-File',
  saveas: 'Save-File',
  primary: 'Turbo-Help-Primary-File',
  'open-options': 'Turbo-Help',
  'save-options': 'Save-Options',
  find: 'Find-Text',
  replace: 'Turbo-Help-Replace',
  goto: 'Turbo-Help-Go-to-Line',
  findproc: 'Turbo-Help-Find-Procedure',
  finderror: 'Turbo-Help-Find-Error',
  compileropts: 'Turbo-Help-Compiler-Options',
  memory: 'Turbo-Help-Memory-Sizes',
  linker: 'Turbo-Help-Linker',
  debugger: 'Turbo-Help-Debugger',
  directories: 'Turbo-Help-Directories',
  params: 'Turbo-Help-Program-Parameters',
  evaluate: 'Turbo-Help-Evaluate-and-Modify',
  'edit-breakpoint': 'Turbo-Help-Add-Breakpoints',
  breakpoints: 'Turbo-Help-Add-Breakpoints',
  startup: 'Turbo-help-startup-options',
  colors: 'Turbo-help-colors-dialog-box',
  tools: 'Turbo-Help-Tools',
  preferences: 'Help-Preferences',
  editoropts: 'Help-Editor-options',
...
const LOCAL_CONTEXTS: Record<string, string[]> = {
  'Find-Text': [
    ...
    '',
    ' Case sensitive matches upper and lower case.',
    ' Choose the direction, scope and origin.',
    ' OK moves to the next matching text.',
    ' Search Again repeats the previous search.',
  ],
  'Open-File': [
    '─',
    '  ▄ Help on the Open a File dialog box',
    '',
    ' Enter a Pascal filename or select a file.',
    ' Open creates an Edit window for that file.',
    ' Replace closes the active window first.',
    ' Imported .PAS files are listed here.',
  ],
  'Save-File': [
    '─',
    '  ▄ Help on the Save File As dialog box',
    '',
    ' Enter a name for the active source file.',
    ' OK saves the source to the virtual disk.',
    ' File Open can retrieve it after a reload.',
  ],
  'Save-Options': [
    '─',
    '  ▄ Help on the Save Options As dialog box',
    '',
    ' Enter a .TP filename for this configuration.',
    ' OK saves the current environment settings.',
...
export function isReferenceHelp(topic: string): boolean {
  return Object.hasOwn(TOPICS, topic) || topic === 'contents';
}
...
export function helpEmphasis(topic: string): [number, number, number, number][] {
  const reference =
    topic === 'edit'
      ? [
          ...(REFERENCE_HELP_EMPHASIS['Help-Edit-Window'] ?? []),
          ...(REFERENCE_HELP_EMPHASIS['Help-2'] ?? []).map(
            ([row, col, length, color]): [number, number, number, number] => [
              row + 16,
              col,
              length,
              color,
      ...
      : (REFERENCE_HELP_EMPHASIS[
          topic.startsWith('context:') ? topic.slice(8) : (TOPICS[topic] ?? '')
        ] ?? []);
  const prefixLength =
    topic === 'edit' ? 32 : topic.startsWith('context:') || Object.hasOwn(TOPICS, topic) ? 16 : 0;
  return [
    ...reference,
    ...getHelpDocument(topic)
      .links.filter((entry) => entry.row >= prefixLength)
      .map(({ row, col, length }): [number, number, number, number] => [row, col, length, 14]),
  ];
}
...
export function navigateHelp(key: string, pageSize: number): boolean {
  ...
  if (key === 'Tab' || key === 'Shift+Tab') {
      ...
      key === 'Tab'
        ? links[
            position >= 0
              ...
              : Math.max(
                  0,
                  links.findIndex((entry) => entry.row >= cursor)
                )
          ]
        : links[
            position >= 0
    ...
    const current = Math.max(
      0,
      links.findIndex((entry) => entry.col === selectedCol)
    );
    ...
    scroll = Math.min(Math.max(0, document.lines.length - pageSize), scroll + pageSize);
    cursor = Math.min(last, scroll + pageSize - 1);
    selectedCol = 0;
  } else if (key === 'PageUp') {
    ...
    cursor = last;
    scroll = Math.max(0, document.lines.length - pageSize);
    selectedCol = 0;
  } else return false;
...
const hasCrossReferences = (topic: string): boolean =>
  topic !== 'context:Turbo-Help-Add-Breakpoints';

function contextDialog(lines: string[], topic: string): DialogDef {
  const button = (id: string, label: string, y: number) => ({
    kind: 'button' as const,
    id,
    label: label + ' '.repeat(Math.max(0, 9 - label.replaceAll('~', '').length)),
    x: 54,
    y,
    w: 11,
    result: id,
    default: id === 'cross',
    disabled: id === 'cross' && !hasCrossReferences(topic),
    hint: 'Navigate the Help system',
  });
  return {
    id: 'context-help',
    title: 'Turbo Help',
    rect: { x: 6, y: 3, w: 68, h: 19 },
    controls: [
      {
        kind: 'help',
        id: 'page',
        ...
        lines,
        topic,
        hint: 'Read help for the current context',
      },
      button('cross', 'Cross ~r~ef', 3),
      button('previous', '~P~revious', 6),
      button('contents', '~C~ontents', 8),
      button('index', '~I~ndex', 10),
...
export function showHelp(
  topic: string,
  modal = useDialogStore.getState().stack.length > 0,
  remember = true
): void {
  if (remember && history.at(-1)?.topic !== topic) {
    const active = useDesktopStore.getState().activeWindow();
    const previous = history.at(-1);
    if (previous && active?.kind === 'help') {
      previous.selected = active.selected;
      previous.scroll = active.scroll;
      previous.col = selectedCol;
    }
  ...
  if (modal) {
    ...
    if (help) {
      desktop.scrollTool(help.id, -help.scroll);
      desktop.setToolSelection(help.id, 0);
    }
  }
}
...
export function showContextHelp(context: string): void {
  ...
  showHelp(
    topic ? `context:${topic}` : context === 'edit' ? 'edit' : 'using',
    useDialogStore.getState().stack.length > 0
  );
}
...
export function followHelpLine(line: string, row?: number, col?: number, modal = false): boolean {
  ...
  const entry: HelpLink | undefined =
    col === undefined
