...
const text = (x: number, y: number, value: string): Control => ({
  kind: 'static',
  x,
  y,
  text: value,
...
export const printerSetupDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~F~ilter path', 'filter'),
    {
      ...
      w: 37,
      history: false,
      hint: 'Enter path and filename of printer filter/driver',
    },
    label(3, 5, '~C~ommand line', 'command'),
    {
      ...
      w: 37,
      history: false,
      hint: 'Enter command line for printer filter/driver',
    },
    {
      kind: 'checks',
      id: 'highlight',
      ...
      y: 8,
      w: 37,
      items: items('~S~end highlighting escape codes'),
      hint: 'Print source text with highlighting',
    },
...
export const findDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~T~ext to find', 'text'),
    {
      ...
      y: 2,
      w: 33,
      hint: 'Enter literal text or regular expression to search for',
    },
    ...
    label(3, 9, 'Scope', 'scope'),
    {
      ...
      y: 10,
      w: 24,
      hint: 'Choose the part of the file to search',
      items: items('~G~lobal', '~S~elected text'),
    },
    label(30, 9, 'Origin', 'origin'),
    {
      ...
      y: 10,
      w: 22,
      hint: 'Choose where the search starts',
      items: items('~F~rom cursor', '~E~ntire scope'),
    },
...
export const replaceDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(7, 4, '~N~ew text', 'replacement'),
    {
      ...
      y: 4,
      w: 33,
      hint: 'Enter the text to replace the search string with',
    },
...
export const gotoLineDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~E~nter new line number', 'line'),
    {
      ...
      y: 2,
      w: 8,
      hint: 'Move cursor to specified line number in file',
    },
    ...okCancelHelp(3, 4, 12),
  ],
});
...
export const openFileDialog = (files: string[], info: string[]): DialogDef => ({
  ...
  controls: [
    ...
    btn('open', 36, 3, 9, '~O~pen', 'Open the selected file in a new Edit window', {
      default: true,
    }),
...
export const primaryFileDialog = (files: string[], info: string[]): DialogDef => ({
  ...
  controls: [
    ...
    { kind: 'input', id: 'name', x: 3, y: 3, w: 28, hint: 'Enter directory path and file mask' },
    label(3, 5, '~F~iles', 'files'),
    fileList('files', files, 'Select the primary program file'),
...
export const findProcedureDialog = (): DialogDef => ({
  ...
  rect: { x: 22, y: 8, w: 36, h: 8 },
  controls: [
    label(3, 2, '~P~rocedure name', 'name'),
    {
      kind: 'input',
      id: 'name',
      x: 3,
      y: 3,
      w: 27,
      hint: 'Enter name of procedure or function to locate',
    },
    ...okCancelHelp(3, 5, 11),
  ],
});
...
export const findErrorDialog = (): DialogDef => ({
  id: 'finderror',
  title: 'Find Error',
  rect: { x: 22, y: 9, w: 36, h: 7 },
  controls: [
    label(3, 2, '~E~rror address', 'address'),
    {
      kind: 'input',
      id: 'address',
      x: 17,
      y: 2,
      w: 13,
      hint: 'Find source location corresponding to error address',
    },
    ...okCancelHelp(3, 4, 11),
  ],
});
...
export const optionsFileDialog = (save: boolean, files: string[]): DialogDef => ({
  ...
  id: save ? 'save-options' : 'open-options',
  title: save ? 'Save Options As' : 'Open Options',
  controls: [
    label(3, 2, '~O~ptions file name', 'name'),
    {
      ...
      y: 3,
      w: 28,
      hint: `Specify disk file name for ${save ? 'saving' : 'loading'} configuration options`,
    },
    label(3, 5, '~F~iles', 'files'),
    fileList('files', files, 'Select an options configuration file'),
    btn('ok', 36, 3, 9, 'O~K~', OK_HINT, { default: true }),
    btn('cancel', 36, 11, 9, 'Cancel', CANCEL_HINT),
    btn('help', 36, 14, 9, 'Help', HELP_HINT),
    {
      ...
      w: 47,
      h: 2,
      lines: ['C:\\TPASCAL\\*.TP', '..           Directory Aug  1, 2017   6:23pm'],
      attr: { fg: C.Cyan, bg: C.Blue },
    },
  ],
});
...
export const changeDirDialog = (tree: string[], selected: number): DialogDef => ({
  ...
  controls: [
    ...
    label(3, 5, 'Directory ~t~ree', 'tree'),
    {
      ...
      h: 9,
      items: tree,
      hint: 'Use cursor keys to navigate the directory tree',
      scroll: 'v',
    },
...
export const windowListDialog = (windows: string[]): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~W~indows', 'windows'),
    {
      ...
      h: 10,
      items: windows,
      hint: 'Use cursor keys to examine windows in window list',
      scroll: 'v',
    },
...
export const compilerOptionsDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, 'Code generation', 'codegen'),
    {
      ...
      items: [
        {
          label: '~F~orce far calls',
          hint: 'Force all procedures and functions to use far call model',
        },
    ...
    label(3, 12, 'Debugging', 'debugging'),
    {
      ...
      y: 13,
      w: 24,
      hint: 'Generate information for the debugger',
      items: items('~D~ebug information', '~L~ocal symbols'),
    },
    label(30, 13, 'Numeric processing', 'numeric'),
    {
      ...
      y: 14,
      w: 27,
      hint: 'Set floating point code generation',
      items: items('~8~087/80287', '~E~mulation'),
    },
    label(3, 17, 'Conditio~n~al defines', 'defines'),
    {
      ...
      y: 18,
      w: 51,
      hint: 'Enter conditional defines separated by semicolons',
    },
...
export const memorySizesDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~S~tack size', 'stack'),
    {
      ...
      w: 9,
      history: false,
      hint: "Size of your program's stack segment (between 1024 and 65520)",
    },
    label(3, 4, '~L~ow heap limit', 'low'),
    {
      ...
      w: 9,
      history: false,
      hint: 'Minimum amount of heap memory your program needs',
    },
    label(3, 6, '~H~igh heap limit', 'high'),
    {
      ...
      w: 9,
      history: false,
      hint: 'Maximum amount of heap memory your program may use',
    },
    ...okCancelHelp(3, 8, 11),
  ],
});
...
export const linkerDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(21, 2, 'Link buffer', 'buffer'),
    {
      ...
      y: 3,
      w: 12,
      hint: 'Choose where the linker keeps its buffer',
      items: items('~M~emory', 'D~i~sk'),
    },
    ...okCancelHelp(3, 8, 11),
  ],
});
...
export const debuggerDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(21, 2, 'Display swapping', 'swapping'),
    {
      ...
      y: 3,
      w: 16,
      hint: 'Choose when to swap to the user screen',
      items: items('~N~one', 'S~m~art', '~A~lways'),
    },
    ...okCancelHelp(7, 7, 11),
  ],
});
...
export const directoriesDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~E~XE & TPU directory', 'exe'),
    {
      ...
      y: 2,
      w: 31,
      hint: 'The directory that stores your .EXE, .TPU and .MAP files',
    },
    label(3, 4, '~I~nclude directories', 'include'),
    {
      ...
      y: 4,
      w: 31,
      hint: 'Directories searched for include files',
    },
    ...
    label(3, 8, '~O~bject directories', 'object'),
    {
      ...
      y: 8,
      w: 31,
      hint: 'Directories searched for .OBJ files',
    },
...
export const programParametersDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~P~arameter', 'params'),
    {
      ...
      y: 2,
      w: 35,
      hint: 'Enter command line parameters to be passed to your program',
    },
...
export const evaluateDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(3, 5, '~R~esult', 'result'),
    {
      ...
      w: 35,
      readOnly: true,
      hint: 'The value of the expression',
    },
...
export const breakpointsDialog = (entries: string[]): DialogDef => ({
  ...
  controls: [
    ...
    label(65, 2, 'Pass', 'list'),
    {
      ...
      h: 8,
      items: entries,
      hint: 'Use cursor keys to examine list of breakpoints',
      scroll: 'v',
    },
    btn('ok', 3, 12, 8, 'O~K~', OK_HINT, { default: true }),
    btn('edit', 14, 12, 8, '~E~dit', 'Edit the selected breakpoint'),
    btn('delete', 25, 12, 8, 'Delete', 'Delete the selected breakpoint', {
      disabled: entries.length === 0,
    }),
    btn('view', 36, 12, 8, 'View', 'View the source of the selected breakpoint', {
      disabled: entries.length === 0,
    }),
    btn('clear', 47, 12, 11, 'Clear all', 'Delete all breakpoints', {
      disabled: entries.length === 0,
    }),
...
export const toolsDialog = (tools: string[]): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~P~rogram titles', 'list'),
    {
      ...
      h: 11,
      items: tools,
      hint: 'Use cursor keys to examine the list of Tools',
      scroll: 'v',
    },
...
export const preferencesDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(26, 2, 'Source tracking', 'tracking'),
    {
      ...
      y: 3,
      w: 27,
      hint: 'Choose where tracked source is displayed',
      items: items('~N~ew window', '~C~urrent window'),
    },
    label(3, 6, 'Auto save', 'autosave'),
    {
      ...
      y: 7,
      w: 21,
      hint: 'Choose what is saved automatically',
      items: items('Editor ~f~iles', '~E~nvironment', '~D~esktop'),
    },
    ...
    label(3, 11, 'Desktop file', 'desktop'),
    {
      ...
      y: 12,
      w: 50,
      hint: 'Choose where the desktop file is kept',
      items: items('C~u~rrent directory', 'Conf~i~g file directory'),
    },
...
export const editorOptionsDialog = (): DialogDef => ({
  ...
  controls: [
    ...
    label(3, 11, '~T~ab size', 'tab'),
    {
      ...
      w: 4,
      history: false,
      hint: 'Number of columns a tab advances',
    },
    label(3, 13, '~H~ighlight extensions', 'ext'),
    {
      ...
      y: 14,
      w: 47,
      hint: 'File extensions that get syntax highlighting',
    },
  ...
  controls: [
    label(3, 2, 'Ctrl+Right mouse button', 'right'),
    {
      ...
      items: [
        ...
        {
          label: '~T~opic search',
          hint: 'Selects topic search for indicated word in editor or help window',
        },
        { label: '~G~o to cursor' },
        { label: '~B~reakpoint' },
        { label: '~E~valuate' },
    ...
    { kind: 'slider', x: 28, y: 4, w: 26, pos: 8, max: 23 },
    {
      ...
      y: 7,
      w: 27,
      hint: 'Swap the left and right mouse buttons',
      items: items('~R~everse mouse buttons'),
    },
  ...
  controls: [
    {
      ...
      items: [
        {
          label: '~D~ual monitor support',
          hint: 'Use dual monitors to run IDE and user program on separate displays',
        },
    ...
    label(32, 3, '~W~indow heap size', 'window'),
    {
      ...
      w: 6,
      history: false,
      hint: 'Kilobytes of memory for the window heap',
    },
    label(32, 5, '~E~ditor heap size', 'editorheap'),
    {
      ...
      w: 6,
      history: false,
      hint: 'Kilobytes of memory for the editor heap',
    },
    label(32, 7, '~O~verlay heap size', 'overlay'),
    {
      ...
      w: 6,
      history: false,
      hint: 'Kilobytes of memory for the overlay heap',
    },
    label(3, 11, '~S~wap file directory', 'swap'),
    {
      ...
      w: 33,
      history: false,
      hint: 'Directory for the IDE swap file',
    },
...
export const colorsDialog = (): DialogDef => ({
  ...
  controls: [
    label(3, 2, '~G~roup', 'group'),
    {
      ...
      h: 11,
      items: COLOR_GROUPS,
      hint: 'Use cursor keys to select an IDE group and customize its colors',
      scroll: 'v',
    },
    label(21, 2, '~I~tem', 'item'),
    {
      ...
      h: 11,
      items: COLOR_ITEMS,
      hint: 'Use cursor keys to select an item and customize its colors',
      scroll: 'v',
    },
    {
      kind: 'swatches',
